...
- **📂 Local folder** — ディスク上の**実フォルダ**を選んで保存先にします（対応ブラウザ＝Chromium 系）。選択時にフォルダのアクセス許可を求められます。
...
#### 差分保存（2026-07 から**既定で有効**）

自動保存は「コンテナ丸ごと」ではなく「**変更したエントリだけ**」を書き込みます（`persistence.differential_save`、2026-07-22 から既定 ON）。編集のたびの保存コストがコンテナの大きさによらず一定になり、数千件規模でも保存が重くなりません。どの保存モード（IndexedDB / OPFS / Local folder）でも同じに動作します。

- **既存データはそのまま読めます**。切替時の操作は不要で、次の自動保存から新形式に切り替わります
- ⚠️ **古い版の PKC2 に戻す場合だけ注意**: 新形式で保存されたストレージを、この機能を知らない**古い版で開くとエントリが空に見えます**（データ自体は消えておらず、新しい版で開けば戻ります）。古い版に戻す前には、Flags Inspector で `persistence.differential_save` を OFF にして一度保存（何か編集）するか、Export しておいてください。OFF にすれば次の自動保存で従来形式に書き戻されます（双方向に安全）
