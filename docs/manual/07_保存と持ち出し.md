...
「Containers (this origin)」で、同一オリジン（同じ URL）に保存された**複数のコンテナ**を切り替えられます。現在のコンテナには ● が付きます。

- **Switch** — そのコンテナに切り替え（再読み込み）
- **+ New container** — 空のコンテナを新規作成して切り替え
- **Delete** — そのコンテナを削除（取り消せません。現在のコンテナは削除できません）

各コンテナは独立した作業空間です。用途別（仕事／私用など）に分けたいときに使えます。持ち出し（Export / 共有）はコンテナ単位なのは従来どおりです。
