...
- **📂 Local folder** — ディスク上の**実フォルダ**を選んで保存先にします（対応ブラウザ＝Chromium 系）。選択時にフォルダのアクセス許可を求められます。許可はセッションごとに切れることがあり、その場合は「Local folder (re-pick…)」で再選択して再許可してください。
...
#### 差分保存（上級者向け・opt-in、2026-07 追加）

数千件規模のエントリを持つワークスペースでは、Flags Inspector（右上 ⚙ → ⚑ Flags）で `persistence.differential_save` を ON にすると、自動保存が「コンテナ丸ごと」から「**変更したエントリだけ**」の書込みに変わり、編集のたびの保存コストが一定になります。

⚠️ **留意点**: ON で保存したストレージを、この機能を知らない**古い版の PKC2 で開くとエントリが空に見えます**（データ自体は消えておらず、新しい版で開けば戻ります）。古い版に戻す前には、この flag を OFF にして一度保存（何か編集）するか、Export しておいてください。OFF に戻せば次の自動保存で従来形式に書き戻されます。
