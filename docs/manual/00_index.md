...
| 07 | 保存と持ち出し | **保存の全体像の図解**(保存先 3 種 × 使えない環境のフォールバック × 保存形式)・Export の概要・ZIP import 警告・ストレージ 3 モードの使い分け(IndexedDB / OPFS / Local folder)・差分保存 / 省容量形式(**いずれも提供終了・自動移行**)・保存領域が使えない環境の案内ダイアログ・UI 設定の持ち出し(Settings File) |
