...
**最終更新**: Issue #21 完了時点（Search/Filter Enhancement）
...
前セッションで Issue #1〜#21 が完了しており、feature 層の最初の実用的 slice が成立しています。
...
テスト: 352 tests 全通過、typecheck clean、lint clean
...
【feature 層の実装方針（#20-#21 で確立）】
- feature 関数は core の型のみ import（adapter を import しない）
- feature state（searchQuery, archetypeFilter）は AppState に runtime-only で直置き
...
├── features/
│   └── search/         # filter.ts (filterEntries, filterByArchetype, applyFilters)
...
tests/                  # 352 tests across 25 test files
...
- i18n 本実装
- sort / fuzzy search / saved filters
- tags / relation の UI 可視化
