...
**最終更新**: Issue #41 完了時点（Attachment Guardrail UX）
...
前セッションで Issue #1〜#41 が完了しており、feature 層の検索・ソート・relation 観測・tags・tag filter・archetype dispatch（todo + form + attachment 実証 + UX polish + quick update 契約明文化）、保存・再水和・可搬モデル（body-assets分離 + IDB assets store + Light/Full export + readonly/rehydrate + gzip圧縮 + ZIP Package）、Attachment Guardrail UX が成立しています。
...
| # | 内容 |
|---|------|
| 35 | Attachment Body-Assets 分離 — entry.body にメタ、container.assets に実データ、backward compat |
| 36 | IDB Assets Store 分離 (Phase 1) — DB version 2、assets object store、ContainerStore 拡張 |
| 37 | HTML Export Light/Full Mode — export_meta 追加、Light は assets 除外、Full は全データ |
| 38 | HTML Readonly Mode + Rehydrate — data-pkc-mode 属性、readonly UI、再水和で Workspace 昇格 |
| 39 | HTML Full Compression — gzip+base64 圧縮（CompressionStream）、asset_encoding、fallback |
| 40 | ZIP PKC2 Package Export/Import — 完全再現型可搬形式、manifest/container/assets構造、自前ZIP実装 |

UX ガードレール (#41):
| # | 内容 |
|---|------|
| 41 | Attachment Guardrail UX — ファイルサイズ警告(1MB/5MB)、Light export 警告、Full export サイズ見積、ZIP 推奨 |

テスト: 693 tests 全通過、typecheck clean
ビルド: 成功
...
- pkc-data shape: { container: Container, export_meta?: ExportMeta }
- export_meta: { mode, mutability, asset_encoding? } — export artifact のメタデータ
...
- ARCHETYPE_LABELS マップで全 archetype に人間可読ラベル（text→Note, todo→Todo, form→Form 等）を統一提供
- sidebar badge, detail/editor ラベル, filter bar, restore candidates すべてが ARCHETYPE_LABELS を参照
- sidebar badge に data-pkc-archetype 属性を追加（CSS スタイリング用）
- detail view / editor の title 横に pkc-archetype-label を表示
- attachment presenter: body にメタデータ（name, mime, size, asset_key）、container.assets に base64 データ
  - body-assets 分離済み（#35）。旧形式（body に data 直格納）も backward compat で読み取り可能
  - IDB では assets が別 object store に分離保存（#36）。save debounce で assets が毎回書き込まれない
  - 制約: 推奨 1 MB 未満、ストリーミング・チャンク・外部ストレージ非対応、単一ファイル/エントリ
- guardrails（adapter/ui/guardrails.ts）: export 前の UX ガードレール
  - classifyFileSize: 1MB soft / 5MB heavy のファイルサイズ分類
  - fileSizeWarningMessage: attachment 追加時のサイズ警告メッセージ
  - lightExportWarning: Light export 時の assets 除外警告
  - fullExportEstimation: Full export のサイズ見積もり（gzip 考慮）
  - zipRecommendation: 大きな assets がある場合の ZIP 推奨
  - renderer の header に export guardrails を data-pkc-region="export-guardrails" で表示
  - attachment-presenter の editor に data-pkc-region="attachment-size-warning" で表示
- HTML export は 4 モード対応（editable/readonly × light/full）
  - Light: assets 除外、テキスト中心なら数十 KB
  - Full: assets を gzip+base64 で圧縮して埋め込み（#39）
  - Readonly: UI ポリシー。再水和ボタンで Workspace に昇格（#38）
- compression: adapter/platform/compression.ts
  - CompressionStream 対応時: gzip+base64（テキスト系 30-55% 削減）
  - 非対応時: 非圧縮 base64 fallback（export は失敗しない）
  - IDB は常に非圧縮 base64（CPU コスト回避）
  - asset_encoding フィールドで export/import の対称性を保証
- ZIP PKC2 Package: adapter/platform/zip-package.ts
  - 完全再現型可搬形式（HTML とは別契約）
  - ZIP 構造: manifest.json + container.json + assets/<key>.bin
  - assets は raw binary（base64 decode して格納）。ZIP 標準ツールで開ける
  - import 時に新 cid 発行（衝突回避、Workspace 独立性）
  - 外部 ZIP ライブラリ不使用（stored mode の最小実装、外部依存ゼロ）
  - manifest に format/version/source_cid/counts を記録
...
│   ├── ui/             # renderer, action-binder, event-log, detail-presenter, todo/form/attachment-presenter, guardrails
│   ├── platform/       # idb-store, persistence, exporter, importer, embed-detect, compression, zip-package
...
tests/                  # 693 tests across 36 test files

【保存・再水和・可搬モデルの到達点（17_保存再水和可搬モデル.md 参照）】
Phase α（設計固定）: 完了
Phase β（基盤改修）: 完了（#35 body-assets 分離 → #36 IDB assets store → presenter migration）
Phase γ（export 拡張）: 完了（#37 Light/Full → #38 Readonly/Rehydrate → #39 gzip+base64 圧縮）
Phase δ（可搬形式）: PKC2 Package ZIP 完了（#40）。Document Set / Template は未着手

【まだ意図的にやっていないこと】
- **ZIP Document Set**（best-effort 再構成形式。将来）
- **Template Package**（概念定義のみ、将来）
- merge import（完全置換のみ）
...
- archetype 管理 UI（設定画面、migration）
- attachment の高度化（複数ファイル、drag & drop、preview、ストリーミング、外部ストレージ）
