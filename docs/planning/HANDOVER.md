...
**最終更新**: Issue #24 完了時点（Tags Minimal UI）
...
前セッションで Issue #1〜#24 が完了しており、feature 層の検索・ソート・relation 観測・tags 基盤が成立しています。
...
テスト: 417 tests 全通過、typecheck clean、lint clean
...
│   └── relation/       # selector.ts (getRelationsForEntry, resolveRelations), tag-selector.ts (getTagsForEntry, getAvailableTagTargets)
