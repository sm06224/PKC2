...
テスト: 531 tests 全通過、typecheck clean、lint clean
ビルド: dist/pkc2.html (56.1 KB) 生成成功
...
- attachment presenter: base64 エンコードでファイルデータを body に保持。file input → hidden fields → collectBody の流れ
  - **暫定実装**: 現在は entry.body に base64 data を直格納。17_保存再水和可搬モデル.md で body/assets 分離が設計済み
  - 制約: base64 は ~33% サイズ膨張、推奨 1 MB 未満、ストリーミング・チャンク・外部ストレージ非対応
  - 単一ファイル/エントリ、preview 機能なし
...
【保存・再水和・可搬モデルの今後（17_保存再水和可搬モデル.md 参照）】
現行 attachment は暫定実装（body に base64 直格納）。以下の順に進化予定:
- Phase β: attachment body-assets 分離 → IDB assets store 分離 → presenter migration
- Phase γ: HTML export Light/Full mode → HTML readonly + 再水和 → gzip+base64 圧縮
- Phase δ（将来）: ZIP PKC2 Package → ZIP Document Set → Template Package
詳細は 17_保存再水和可搬モデル.md の 17.10 推奨実装順序を参照。

【まだ意図的にやっていないこと】
- **body-assets 分離**（設計済み、未実装。17番文書参照）
- **HTML export modes**（editable/readonly × light/full。設計済み、未実装）
- **gzip+base64 圧縮**（設計済み、未実装）
