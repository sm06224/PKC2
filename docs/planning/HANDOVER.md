...
**最終更新**: Issue #44 完了時点（Pre-Release Snapshot Stabilization）
...
Issue #1〜#44 が完了し、保存・再水和・可搬の基幹線が完成しています。
現在はプレリリース段階（v0.1.0）です。これ以上の機能拡張より安定化を優先してください。
...
- 4種の archetype（Note, Todo, Form, File）で構造化された知識管理
...
│   │                   # todo/form/attachment-presenter, guardrails
...
│   └── relation/       # selector.ts, tag-selector.ts, tag-filter.ts
...
【ArchetypeId】
'text' | 'textlog' | 'todo' | 'form' | 'attachment' | 'generic' | 'opaque'
