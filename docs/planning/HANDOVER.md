...
テスト: 672 tests 全通過、typecheck clean
ビルド: 成功
...
│   ├── platform/       # idb-store, persistence, exporter, importer, embed-detect, compression
...
Phase δ（可搬形式）: 未着手（ZIP PKC2 Package → ZIP Document Set → Template Package）

【まだ意図的にやっていないこと】
- **ZIP export/import**（設計済み、将来）
