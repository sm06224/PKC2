...
**最終更新**: Issue #29 完了時点（Todo Usability Minimum）
...
前セッションで Issue #1〜#29 が完了しており、feature 層の検索・ソート・relation 観測・tags・tag filter・archetype dispatch（todo 実証 + editor commit 整理 + todo usability minimum）が成立しています。
...
テスト: 481 tests 全通過、typecheck clean、lint clean
...
- QUICK_UPDATE_ENTRY: ready phase でbody のみ更新（title 保持、snapshot 作成）— quick toggle 等に利用
...
│   ├── ui/             # renderer, action-binder, event-log, detail-presenter, todo-presenter
...
tests/                  # 481 tests across 31 test files
...
- archetype 別 presenter 拡充（form, textlog 等）
