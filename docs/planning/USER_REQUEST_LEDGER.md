...
| **ユーザー明示要望 — 完了** | **25** |
| **ユーザー明示要望 — 部分完了** | 0 |
| **ユーザー明示要望 — 未完（本セッション着手）** | **1（A-2 昇格）** |
| 待機中候補（idea / vision / HANDOVER decline、§3） | 27（重複込 31） |

**結論**: supervisor が台帳 §3 から **A-2（text split edit in entry window）**
を正式に「昇格」させて今セッションで実装する運用に移行。A-2 を §3.1 から
§1 の `S-13` に移す（PROMOTED）。他の 27 候補は昇格条件付きで据え置き。
