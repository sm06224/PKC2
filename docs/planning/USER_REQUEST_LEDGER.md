...
| S-44 | **Dead-code inventory after relations wave**: relations wave 後の未使用 export 監査 + 同 PR 内で Category A 2 件（`isMarkdownEvaluatedArchetype` inline 化 + 1 件削除）を解消。Category B / C / D は空 | supervisor prompt「relations wave 後の未使用 export 監査」 | 完了 | `../development/archived/dead-path-cleanup/dead-code-inventory-after-relations-wave.md`（audit + cleanup） | — |
...
| P-6 | TODO / attachment の自動整理（auto-placement） | `archived/singletons/auto-folder-placement-for-generated-entries.md` (#26) | 完了 | CLOSED dev doc |
...
| 2026-04-21 | **S-44 / Dead-code inventory after relations wave 完了**: relations wave 後の未使用 export 監査 + 同 PR 内で Category A 2 件（`isMarkdownEvaluatedArchetype` inline 化 + 1 件削除）を解消。Category B / C / D は空。実装 doc: `../development/archived/dead-path-cleanup/dead-code-inventory-after-relations-wave.md`（audit + cleanup） |
...
| 2026-05-07 | **Wave 10-9 stabilization closure — 修正指示 1〜10 + 連続報告全件着地（Δ1〜Δ34、122 commit / 100 PR）**。領域 10-6(filer / graph / inventory)着地後の連続 hotfix wave、user 判断「いくつかのバグ挙動はあるが締める」で wave クローズ。主軸 8 領域(filer 行ピクセル / graph 描画 + 操作 / inline-calc / ZIP import streaming / popup sync / multi-select 独立 / Flags caption 即時 / 右クリック menu)。残バグは bundle.css 146 KB(budget 超過、次 wave 吸収)/ rubber band 2-hop 止まり / drag position 永続なし(別 wave)。詳細:[`../development/wave-10-9-stabilization-summary.md`](../development/wave-10-9-stabilization-summary.md) + [`../development/completed/codespaces-merge-playbook-wave-10-9.md`](../development/completed/codespaces-merge-playbook-wave-10-9.md) + `../release/CHANGELOG_v2.2.0.md` Wave 10-9 stabilization セクション。次 wave 候補は 10-2 / 10-3 / 10-4 |
