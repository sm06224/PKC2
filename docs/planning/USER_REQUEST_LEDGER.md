...
| **ユーザー明示要望 — 部分完了** | 0 |
| **ユーザー明示要望 — 未完（着手中）** | 0 |
| 待機中候補（idea / vision / HANDOVER decline、§3） | 24（重複込 28、A-4 部分昇格分は依然として残置）|
...
| A-4 | `search-ux-partial-reach.md` | **PARTIALLY PROMOTED — Slice α 完了（§1 S-15、2026-04-14）/ 残り Slice β γ は CONDITIONAL** | 検索が body 内 sub-location を指せない問題。**Slice α（visual `<mark>` のみ）を最小差分で実装**。indexer / sub-location action / scroll-to / temporary highlight アニメーション / per-result row UI は依然 CONDITIONAL | 「mark だけでは足りない / 該当箇所まで自動スクロールしてほしい」とユーザーが追加報告 |
...
|---------|------|---------------|---------|
| A (immediate UX) | 4 | 2（A-1 / A-3）+ 1（A-2 = §1 S-13）+ 1（A-4 Slice α = §1 S-15）= 4 | A-4 の Slice β / γ は依然 CONDITIONAL |
| B (markdown ext) | 3 | **2（B-1 = §1 S-16、B-2 = §2 P-13 retroactive）** | 1（B-3 のみ） |
