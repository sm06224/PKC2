...
| 案 | 評価 |
|----|------|
| localStorage 単独 | container 移動で失われる、AGPL 単一 HTML 哲学と部分的不整合 |
...
→ **採用**: `__flags__` system entry を **canonical 永続層** とし、URL / localStorage を上位 layer として併用(§3 Resolution order)。

### 0-3. 方向性

PKC2 の single-HTML / self-describing / self-configuring container 哲学に整合させ、**flag 値も Container 内に保持**(`__settings__` と同 idiom)。同時に **debug / dev / power user のための ephemeral / browser-wide layer** を URL / localStorage で薄く追加。
...
- 4 layer の resolution order(URL > localStorage > Container > default)を確立(本 spec の §3)
- flags inspector overlay の最小実装(`?pkc-flag=*` で起動、shell-menu からも到達可)
...
2. localStorage       'pkc-flags' JSON in localStorage  (browser-wide, persistent)
3. Container          entry[lid='__flags__'].body.values[KEY]  (per-container, sharable)
...
| Layer | 読み | 書き | 永続性 |
|---|---|---|---|
| URL | parse on boot | URL 編集のみ(inspector では不可) | session 限定 |
| localStorage | boot 時に 1 回読込 + 変更時 cache | inspector / API で可 | browser ごと |
...
| Tier | URL read | localStorage R/W | `__flags__` R/W | inspector 表示 | 値変更 |
|---|---|---|---|---|---|
| 0 | ✅ | ✅ | ✅ | ✅ | inspector / API で可 |
...
- shell-menu link: 「⚑ Flags」button(既存「🐞 Report」の隣)、click で URL に `?pkc-flag=*` を append(reload なし、URL hash + history push で吸収)
...
- **List**(category 別 grouping):
  - 各 flag 行: key / current value / default / source(URL / local / container / default、color-coded)/ range・enum / description
  - 編集 UI: numeric は input、boolean は checkbox、enum は dropdown
  - 編集時に「どの layer に書く?」確認 popup(local / container 選択、URL は read-only)
- **Footer**: register flag 総数 / Tier 別件数

### 5-3. 編集経路

| Tier | 編集 UI | 書込先 |
|---|---|---|
| 0 | input / checkbox / dropdown(enabled) | localStorage か `__flags__` を user 選択 |
...
| I-FLAGS-3 | resolution order は `URL > localStorage > Container > default` で固定、layer 跨ぎでの順序変動なし |
