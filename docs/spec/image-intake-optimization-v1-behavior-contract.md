...
Status: 実装済み(v2.1.0 以前に landing、v2.1.1 時点で稼働中)。本書は behavior contract / historical design record として保持。実装の現物は `src/features/image-optimize/` / `tests/features/image-optimize/` / ユーザー向け説明は `../manual/05_日常操作.md` §画像の自動最適化。
...
- **rev.1 (2026-04-19)**: 初版。D-IIO1..D-IIO7 に基づく behavior contract
- **rev.1.1 (2026-04-19)**: supervisor post-review による 3 点補強
  - 数値は v1 default（fixed eternal ではない）と明記（§2-5 / §5-4）
  - remembered optimization の silent 実行を条件付き許可に精緻化（§4-2）
  - `__original` suffix が storage-layer naming convention であり user-facing label ではないことを明記（§3-1 / §4-5）
...
以下は **v1 default parameter** であり、**fixed eternal constant ではない**。実測（圧縮率・視認品質・エッジケース）のフィードバックで実装フェーズ以降に見直し可能な **tunable default** として扱う。behavior contract としては「これらの default 値を持つこと」と「値を変更する際は contract 改定を伴うこと」を固定する。

| パラメータ | v1 default | 根拠 | 見直し余地 |
|-----------|-----------|------|-----------|
| **出力フォーマット** | `image/webp` | lossless/lossy 両対応、透過保持（v1 では不透過のみ対象だが）、モダンブラウザ広範対応 | ブラウザ対応・品質論点が出た場合 |
| **DEFAULT_WEBP_QUALITY** | `0.85` | WebP lossy quality 85。スクリーンショット（文字含む）で視認劣化が最小限、圧縮率 50-80% | 実測で文字潰れ / モザイク感が確認されたら調整 |
| **DEFAULT_MAX_LONG_EDGE** | `2560` px | ウルトラワイド（3440×1440）を縮小対象にする。4K（3840×2160）も対象。Full HD（1920×1080）はそのまま | 4K 監視用途 / スマホ縦長比率で問題があれば調整 |
| **OPTIMIZATION_THRESHOLD** | `512 * 1024`（512 KB） | 512 KB 未満の画像は最適化不要。小アイコン / UI 切り抜きを保護 | §5-4 参照 |

**実装時の扱い**: 上記は TypeScript 定数として `src/adapter/ui/image-optimize/config.ts` 等の単一ファイルに集約し、テスト / 実測で見直した際にそこだけ変更すれば済む構造にする。ハードコーディングを複数箇所に散らさない。
...
> **命名の層について（supervisor 補強）**
>
> `__original` suffix は **storage-layer naming convention** である。すなわち `container.assets` の key 空間で一意性を保つための内部識別子であって、**user-facing label ではない**。
>
> UI 上での表示は人間向けの表現を用いる：
> - attachment detail view: 「**Original kept** (2.9 MB / PNG)」「**Optimized** (450 KB / WebP)」等
> - toast / confirmation dialog: 「原画を保持しました」「最適化版のみ保存しました」等
> - ダウンロードリンクのファイル名: 元のファイル名に `-original` suffix を付与するなど、user-readable な表現を使う
>
> 内部命名（`__original`）が UI 表面に露出することは avoid する。将来 suffix 規則を変更する場合でも UI 文言は影響を受けない設計を担保する。
...
### 4-1-1. Remembered optimization の silent 実行条件（supervisor 補強）

設定記憶に基づく silent 最適化は「完全無告知」ではなく、以下の **全条件** を満たした場合のみ許可する：

| # | 条件 | 目的 |
|---|------|------|
| C1 | **ユーザが明示的に opt-in 済み** | 「今後も同じ設定を使う」チェックボックスが過去に user action で ON になった履歴があること。デフォルト OFF、黙って有効化しない（I-BC12 の延長） |
| C2 | **同じ surface class 内でのみ適用** | paste で記憶した設定は paste にのみ適用。drop / attach には別立てで記憶する。surface 間を跨いだ自動流用は禁止 |
| C3 | **原画保持の有無も記憶される** | action（optimize / decline）だけでなく keepOriginal の真偽値もペアで記憶。後から意図しない損失を避ける |
| C4 | **現在設定を UI 上で確認・変更できる** | 設定画面または attachment 関連 UI で、各 surface class ごとの現在設定を表示し、リセット・変更できる（v1 実装では最小限でよい。§4-4 参照） |
| C5 | **1 回ごとの結果通知は最小限残す** | silent だからといって「何も通知しない」ではなく、info toast で「画像を最適化して保存しました（2.9 MB → 450 KB）」を表示する。ユーザが「何が起きたか」を事後確認できる |

**全条件を満たさない場合**、silent 実行してはならない（= 確認 UI を表示するか、pass-through する）。

### 4-1-2. 設定記憶の storage schema

surface class ごとに独立した preference を localStorage に保存する：
...
- surface class key: `paste` / `drop` / `attach`
- `action`: `"optimize"` | `"decline"`
- `keepOriginal`: boolean
- `rememberedAt`: ISO 8601 timestamp（設定画面での表示に使用）
- 未保存 surface は確認 UI を表示する（記憶なし状態）
...
localStorage 上の schema は §4-1-2 に定義する（surface class ごとに独立した preference を保持）。

### 4-4-1. 現在設定の可視化（C4 要件）

silent 最適化の条件 C4（「現在設定を UI 上で確認・変更できる」）を満たすため、v1 では **最小限** 以下のいずれかを用意する：

- 設定画面が v1 時点で存在する場合: 「画像最適化設定」セクションで surface ごとの preference を表示、リセットボタンを配置
- 設定画面がない場合: attachment 関連 UI のどこか（例: attachment detail view の footer、または menu command）で「最適化設定をリセット」できるエントリポイントを 1 つ設ける

最小限の要求は「ユーザが自分の現在設定を知る手段があること」と「リセットできること」の 2 つ。UI の豪華さは v1 scope 外。

### 4-5. 原画アクセス経路

原画保持が有効な場合、attachment detail view に以下を表示する：

- 「**Optimized** (450 KB / WebP)」のバッジ（現在表示中が最適化版であることの明示）
- 「**Original kept** (2.9 MB / PNG)」セクション（原画があることの表示）
  - 「Original を表示」リンク
  - 「Original をダウンロード」リンク → ブラウザの download 機能で保存（ファイル名は元のファイル名ベース）

**内部命名との分離**: 上記 UI ラベル（`Optimized` / `Original kept` 等）は storage-layer の asset key（`__original` suffix）とは独立して定義する。v1 実装では attachment detail presenter 内に表示用文言を localize 可能な形で持ち、storage key をそのまま UI に流さない。

v1 では attachment detail presenter に上記要素を追加する。textlog 内の inline 画像は常に最適化版を参照する。

### 4-6. toast 通知

「設定に基づく silent 最適化」時の事後通知は **必須** である（§4-1-1 C5）。silent を「無告知」と同義にしてはならない。

| シナリオ | toast 種別 | メッセージ例 | 必須性 |
|---------|-----------|------------|-------|
| 最適化実行（設定に基づく silent） | `info` | `画像を最適化して保存しました: 2.9 MB → 450 KB` | **必須**（C5） |
| 最適化実行（確認 UI からの明示） | なし | 通知不要（ユーザが直前に選択している） | 不要 |
| pass-through（閾値未満） | なし | 通知不要 | 不要 |
| pass-through（透過画像） | `info` | `透過画像のため最適化をスキップしました` | 任意 |
| pass-through（サイズガード） | `info` | `この画像は既に十分小さいため、最適化をスキップしました` | 任意 |
| 最適化失敗 | `warn` | `画像の最適化に失敗しました。元のまま保存します` | **必須** |
| pass-through（GIF/SVG/対象外） | なし | 通知不要 | 不要 |
...
v1 の OPTIMIZATION_THRESHOLD = 512 KB は **v1 default** であり、**fixed eternal constant ではない**。§2-5 の WebP quality / MAX_LONG_EDGE と同じ扱いで、実運用フィードバックで調整する余地を持つ。ただし：

- 変更時は behavior contract の改定を伴う
- 閾値を下げすぎると小画像の不必要な変換が増える
- 閾値を上げすぎると 1 MB 帯のスクリーンショットを見逃す
- 実装上は §2-5 と同じ config ファイルに集約し、ハードコーディングを散らさない
