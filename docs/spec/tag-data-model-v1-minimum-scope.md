...
**Status**: W1 Tag wave 着地済み(2026-04-23)。`entry.tags?: string[]` schema(Slice B)・Tag filter 軸(Slice D)・`tag:` parser が実装済み。Saved Search `tag_filter_v2` schema additive / Color tag wave / UI chip は次 wave。schema / normalization / migration の参照源として有効。既存データ・既存 UI・既存 filter 挙動はすべて不変。
