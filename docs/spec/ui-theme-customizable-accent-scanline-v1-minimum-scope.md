...
**v1 では未対応（runtime-only）。**

セッション内でのみ設定が有効で、ページリロード後はデフォルト（OFF）に戻る。  
localStorage / IndexedDB への保存は、将来の設定 FI で統一的に設計する。  
今 localStorage を入れると AppState との二重管理・rehydrate との不整合・将来の設定機構との衝突が発生する。
...
新 CSS 変数 `--c-kanban-select-bg` をテーマ変数として切り出す（将来の accent color 変更・設定化のフック点になる）。

| フェーズ | 扱い |
|---------|------|
| v1 | CSS 定数として固定（設定なし） |
| v1.x | `--c-kanban-select-bg` 経由でテーマ依存 or 設定化の余地あり |
...
## 5. Runtime state モデル（概要）

```
初期値:
  AppState.showScanline = false（デフォルト OFF）
  → renderer: #pkc-root に data-pkc-scanline 属性なし

設定変更時:
  TOGGLE_SCANLINE dispatch → reducer → AppState.showScanline 更新
  → renderer が #pkc-root に data-pkc-scanline="on" / 属性削除を反映
```

AppState への格納は UI 表示のために必要（`lightSource` と同じ扱い）。  
セッション限り。persistence は v1 では提供しない。
...
| persistence | `pkc-pane-prefs`（localStorage） | v1 では未対応（runtime-only）|
...
| Persistence（localStorage）| v1 では不採用。単一 HTML 哲学・rehydrate・将来設定 FI と衝突する |
