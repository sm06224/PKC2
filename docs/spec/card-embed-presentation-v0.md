...
- **card presentation**: **spec accepted、parser helper + renderer placeholder + click / keyboard wiring 実装済み**(2026-04-24 Slice 1 / Slice 2 / Slice 3.5、2026-04-25 Slice 4、`src/features/link/card-presentation.ts` + `src/features/markdown/markdown-render.ts` の `pkc-card` core rule + `src/adapter/ui/action-binder.ts` の `navigate-card-ref` case + `runEntryRefNavigation` 共有 helper)。`@[card](<target>)` は `<span class="pkc-card-placeholder" data-pkc-action="navigate-card-ref" data-pkc-card-* role="link" tabindex="0">` に変換され、click / Enter / Space で対象 entry に遷移する(同一 container `entry:<lid>` および `pkc://<self>/entry/<lid>` のみ navigate、cross-container `pkc://<other>/entry/<lid>` は silent no-op)。**target は `entry:<lid>` / `pkc://<cid>/entry/<lid>` のみ canonical**、`asset:<key>` / `pkc://<cid>/asset/<key>` は Slice-3.5 で parser からも reject(spec §5.4 / audit Option C と整合)。**widget UI / thumbnail / excerpt は未実装**(Slice 5 以降)
...
- **v0 で clickable-image を canonical にしない**(`clickable-image-renderer-audit.md` §11.2 Option B の判断を継承)
- migration scanner v1 は **clickable-image を生成も検出もしない**(既存契約、`link-migration-tool-v1.md` §14.1)
- `[![]](target)`(空 inner image)は **invalid / do-not-emit**(同 §14.2)
- `[![alt](asset:<key>)](asset:<key>)` の nested 形は現行 renderer では **literal 漏れ**(`SAFE_URL_RE` に `asset:` 無し、`clickable-image-renderer-audit.md` §4.1)、将来 renderer 対応後に migration v2 で昇格候補
- **本書の責務は card 側**。clickable-image の実装は別 audit doc で航路固定済み
- **Decision audit 続編(2026-04-25)**: `../development/clickable-image-v2-decision-audit.md` で **5/8 form は現 main で既に動作** していること(`[![alt](asset:k)](entry:lid)` 等)を empirical に確認、`asset:` outer link のみ broken。採用方針は **Option B+ ハイブリッド**(動作 form を spec で canonical 化、broken 1 form は future v2 wave で envelope 設計してから実装)。**card と clickable-image は引き続き独立**、相互影響なし
