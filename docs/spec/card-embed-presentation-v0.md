...
- **card presentation**: **spec accepted、Slice 1-4(parser + renderer placeholder + click/keyboard wiring)+ Slice 5.0(minimal chrome)+ Slice 5.1(XSS-safe excerpt)実装済み**(2026-04-24 Slice 1 / 2 / 3.5、2026-04-25 Slice 4 + 5.0 + 5.1、`src/features/link/card-presentation.ts` + `src/features/markdown/markdown-render.ts` の `pkc-card` core rule + `src/adapter/ui/action-binder.ts` の `navigate-card-ref` case + `src/features/card/widget-presenter.ts` + `src/features/card/excerpt-builder.ts` + `src/adapter/ui/card-hydrator.ts`)。`@[card](<target>)` は renderer が placeholder span を emit し、4 つの presenter(detail / folder / todo / textlog)が `expandTransclusions` の直後に `hydrateCardPlaceholders` を呼んで widget chrome(badge + title + excerpt)に hydrate。click / Enter / Space で対象 entry に遷移する経路は Slice 4 の `runEntryRefNavigation` に委譲(missing / cross-container / malformed は `aria-disabled="true"` + `tabindex="-1"` で safe no-op)。**excerpt は plain-text only**、archetype 別の source(TEXT body / TEXTLOG の log / TODO description / FORM 先頭 string field / FOLDER body)を `escapeHtml` 経由ではなく **DOM の `textContent` で流し込む**、markdown-it / asset-resolver 経路を一切通さないため XSS 経路は構造的に成立しない。**target は `entry:<lid>` / `pkc://<cid>/entry/<lid>` のみ canonical**、`asset:<key>` / `pkc://<cid>/asset/<key>` は Slice-3.5 で parser reject + Slice 5.0 hydrator で defence-in-depth malformed 化(spec §5.4 / audit Option C と整合)。**thumbnail(Slice 5.2)/ advanced variants(`compact` / `wide` / `timeline`、Slice 6)は未実装**
...
- `../development/archived/audits-2026-04/link-system-audit-2026-04-24.md` — Copy / Paste / Render / Receive 棚卸し
...
| body 内に残った `<base>#pkc?...` External Permalink を context に応じ internal reference / placeholder に整理 | body residue rendering(`archived/audits-2026-04/link-system-audit-2026-04-24.md` G8) |
...
- `../development/archived/audits-2026-04/link-system-audit-2026-04-24.md`(Copy / Paste / Render / Receive 棚卸し、G1-G8 gap list)
