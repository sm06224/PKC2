...
- **card presentation**: **spec accepted、未実装**(本書で記法固定、renderer hook は未実装)
...
| **card presentation** 記法(`@[card](...)`) | **spec accepted**(本書で固定)、**実装未着手**(Slice 1 以降) |
| card variant(`@[card:compact]` 等) | spec reservation(v0 では variant 無しのみ canonical) |
