...
### 8.3 Saved Search `color_filter`(**実装済み 2026-04-25 — Color Slice 2**)

- `saved-searches-v1.md` に additive 追加:`SavedSearch.color_filter?: string[] | null`(`string[]` 型で round-trip preservation、unknown palette ID も保持)
- 配列は順序意味なし(OR 軸の Set を serialize)が、writer は **dedup → known IDs を palette order でソート → unknown IDs を入力順で末尾 append** の canonical 化を行う(diff / round-trip 安定化のため)
- 旧 reader は unknown field として無視(additive、Tag 軸と同じ)
- 旧 writer から書かれた既存 saved search に `color_filter` が無い場合は Color 軸 OFF として読み込み(欠落 = 無効)
- `createSavedSearch` / `applySavedSearchFields` は Color 軸対応に拡張済み(`SavedSearchSourceFields.colorFilter: ReadonlySet<string>` 必須化、AppState `state.colorTagFilter` Slice 3 着地までは `new Set<string>()` ブリッジ)
- **landing**: `src/features/color/color-palette.ts` + `src/core/model/saved-search.ts` + `src/features/search/saved-searches.ts` + tests `tests/features/color/color-palette.test.ts` / `tests/features/search/saved-searches.test.ts`
