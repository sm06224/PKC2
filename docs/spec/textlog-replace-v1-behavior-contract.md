...
Related: docs/spec/textlog-replace-feasibility-and-minimum-scope.md, docs/spec/find-replace-behavior-contract.md, docs/development/textlog-foundation.md, src/features/textlog/textlog-body.ts, src/adapter/ui/textlog-presenter.ts, src/features/text/text-replace.ts
Supersedes: —
...
| `docs/development/textlog-foundation.md` | textlog 全体仕様。本書の不変条件はここで定義された `TextlogBody` / `TextlogEntry` schema を破壊しないことを保証する |
...
### 4.2 v1 で意図的に持ち込まないもの

- **Selection only**: TEXT contract（v1.1）にはあるが、textlog v1 では搭載しない
  - 理由: 「current line only」がそもそも対象範囲を 1 log に閉じている。Selection only は同じ log 内の更なる範囲限定になるが、実装複雑度に見合うユースケースが整理できていない
  - 将来検討: log 内 selection only は v1.x 内で additive に追加可能（本書の不変条件を壊さない）
- **Replace next / Replace prev**: 一括 Replace All のみ
- **whole word / multiline toggle / preserve case**: TEXT contract と同様に v1 では非搭載
...
`<scope>` は固定で **`current log`**。v1 では Selection only がないので scope バリアントは存在しない。
...
| `docs/development/textlog-foundation.md` | textlog 全体仕様、`TextlogBody` / `TextlogEntry` schema |
| `docs/development/text-replace-current-entry.md` | TEXT 側の実装 dev doc、層分離パターンの参考 |
