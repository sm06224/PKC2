...
| `color_filter?: string[] \| null` | Color ID 配列(`string[]` 型で unknown palette ID も round-trip 保持、known IDs は palette order でソート、unknown は末尾に preserve、空配列は writer で omit) | `undefined` / `[] / null` → Color filter 無効 | Color tag slice 2 + 4(Slice 2 で schema、Slice 4 で `color:<id>` query parser + actual filtering + AppState `state.colorTagFilter` 配線 + Saved Search bridge を実装)**【全件着地済 2026-04-25】** |
...
## 9. Bucket-hide / Unused-attachment 軸（PR #174 追補, 2026-04-27）

§4 で固定した 5 軸とは独立した、**list-shape lens** の 3 トグル
が `<details data-pkc-region="advanced-filters">` の中に追加され
た。これらは「結果集合からどのエントリを更にハイドするか」の
post-filter として動き、`applyFilters` の AND-by-default パイプ
とは別レイヤで掛かる。

| state field | UserAction | default | スコープ |
|---|---|---|---|
| `treeHideBuckets?: boolean` | `TOGGLE_TREE_HIDE_BUCKETS` | `true` | tree mode + flat mode 両方で ASSETS / TODOS フォルダ自体 + 配下を非表示 |
| `searchHideBuckets?: boolean` | `TOGGLE_SEARCH_HIDE_BUCKETS` | `true` | フィルタ active 時、結果からバケット直下のエントリを除外 |
| `unreferencedAttachmentsOnly?: boolean` | `TOGGLE_UNREFERENCED_ATTACHMENTS_FILTER` | `false` | クリーンアップ用の lens — 他から `entry:<lid>` でも `asset:<key>` でも参照されていない attachment 配下のみ surface (`treeHideBuckets` を override) |
| `advancedFiltersOpen?: boolean` | `TOGGLE_ADVANCED_FILTERS` | `false` | 上記 3 + show-archived の `<details>` 開閉 state |

**§4 五軸との関係**: §4 の 5 軸 (FullText / Type / Tag / Color /
Categorical-peer) は「entry 集合をどう絞るか」のセマンティクス。
本節の 3 トグルは「絞ったあとの集合からどう間引くか」の後段
filter で、AND ではなく **段階的合成** として記述する:

```
result = bucketHide(unreferencedLens(applyFilters(entries, axes)))
```

`unreferencedLens` は ON のとき `treeHideBuckets` を bypass する
(クリーンアップ flow の妨げになるため)。OFF のときは
`treeHideBuckets` が先に bucket-folder + 配下を落とし、その後
`searchHideBuckets` が search-active 時のみ個別 entry を落とす。

**Saved Search との関係**: 本 3 トグルは saved search に round-trip
**しない** (runtime-only)。理由は「クリーンアップ flow は per-
session の判断であって "保存したい状態" ではない」。`showArchived`
と同じ runtime-only ポリシーに揃えた。

**実装**:
- helper: `collectUnreferencedAttachmentLids(container)` (`features/asset/asset-scan.ts`)
- placement filter: `getStructuralParent` で walk
- toggle UI: 全部 `<details data-pkc-region="advanced-filters">` 内、default 折り畳み

詳細仕様: `../development/tree-hide-buckets-and-advanced-filters-v1.md` /
`../development/unreferenced-attachments-cleanup-v1.md`
