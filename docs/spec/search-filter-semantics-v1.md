...
| **3** | **Tag**(自由文字列) | **予約: `state.tagFilter: Set<string>`** | `Set<string>`(各要素は Slice B §4 の normalized string) | ❌ 未実装(W1 Slice B docs のみ) | `entry.tags?: string[]` |
...
- **Tag(3)**: **本 draft で予約**。Slice B で entry に `tags?: string[]` が載ったときに、`Set<string>` として filter を受け取る。空 Set / 欠落 → 軸無効。**AND-by-default**(§4.2)。
...
| prefix | 意味 | 対応軸 | 値 | 例 |
|---|---|---|---|---|
| `tag:` | Tag 絞り込み | Tag(§3 軸 3) | normalized string | `tag:urgent` / `tag:"to review"` |
...
| `rel:` | relation kind 絞り込み [reserved、本 draft では parser も軸も実装しない] | §3.3 の reserved | `RelationKind` | `rel:semantic` / `rel:categorical` |
...
現状の `searchQuery` は `tag:urgent` を入力されても **単なる全文検索文字列** として扱い、`title`/`body` に文字列 `"tag:urgent"` が含まれる entry を hit する。parser 実装着地までこの挙動を維持する(破壊的変更でないよう注意)。
...
| `color_filter?: ColorTagId[] \| null` | Color ID 配列 | `undefined` / `[] / null` → Color filter 無効 | Color tag slice(実装 slice 2) |
...
### Slice D — Tag filter data-path 実装(最小、Slice B と本 draft に沿って)

- `state.tagFilter: Set<string>` を additive に追加
- `SET_TAG_FILTER` / `CLEAR_TAG_FILTER` action 追加(Rename slice で名前は空いている)
- `matchesTag(entry, tagFilter)` の AND semantics で `filterActiveAxes` に接続
- `hasActiveFilter` 判定を追加軸に対応
- UI chip / filter bar は手を付けない(CLI / reducer テストだけで動作保証)

### Slice E — Saved Search schema additive(`tag_filter_v2` 追加)
...
- Slice D と順序独立、docs → Slice D 着地後に schema 拡張

### Slice F — Tag chip / filter bar UI prototype

- meta pane の Tag chip 入力 + sidebar filter bar の Tag section
- AND-by-default を UI chip の見た目(チップ横並び)で表現
- W1 Slice A の label / avoid wording を厳守

### Slice G — Search syntax parser draft (docs + impl、後)

- `docs/spec/search-syntax-parser-v1.md` で BNF / tokenizer を固定
- 実装は `src/features/search/parse-query.ts` 想定
- prefix 5 種(`tag:` / `color:` / `type:` / `rel:` / FullText)と quote を正規 parser で認識

推奨: **D → E → F → G**。D は最小で実装できる(既存 filter 合成パターンを踏襲)、E は D 着地後に schema 側を追従、F で UI、G で parser。
...
**Status**: docs-only、search / filter semantics draft。本 draft が固まったので、Slice D 以降の実装 slice は "軸 / AND-OR / prefix 予約 / Saved Search への写像 / visibility 責務" の判断基準をこの 1 本から参照できる。既存 data / UI / filter 挙動はすべて不変。
