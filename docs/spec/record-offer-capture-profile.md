...
### 10.7 既存 reducer への変更点（implementation PR で行う想定）

本 profile で固定した accept 時の body header injection を、**既存 `ACCEPT_OFFER` reducer の 1 箇所**（`src/adapter/state/app-state.ts` の該当 case）に差し込む。具体的な実装は implementation PR で fix するが、本 profile は以下を規定する:
