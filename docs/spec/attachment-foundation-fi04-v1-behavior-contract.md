...
```html
<div
  data-pkc-region="file-drop-zone"
...
`data-pkc-persistent-drop-zone="true"` を付与することで、既存の center pane ドロップゾーン（`data-pkc-region="file-drop-zone"` のみ）と視覚的スタイルで区別できるようにする。イベント処理は共通。
...
`handleFileDropOver` / `handleFileDropLeave` / `handleFileDrop` はすでに `target.closest('[data-pkc-region="file-drop-zone"]')` でターゲットを絞り込んでいる。常設ゾーンも同じ `data-pkc-region` を持つため、**追加の event listener は不要**。
