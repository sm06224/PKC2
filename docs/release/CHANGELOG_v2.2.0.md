...
### Wave Z final — v2.3.0 release-締め wave(2026-05-16〜17、PR #452 + #454)

v2.3.0 release 直前の **締め wave**(user direction「ALL!! DEEP!! GO AHEAD!!」「監査・最適化・ドキュメント整理を行い手順を示して」「ドキュメント整理は通常よりも重視・重厚に行なってください」「これの完了を本 wave の完了とする」)。**PR-W11〜W19**(Wave Z.1 + Z.2、PR #452 で main 着地済)+ **PR-W20〜W24**(Wave Z.3 + Z.4、PR #454 stack 内)で計 14 PR を着地。

- **PR-W24 v6 manual 大改修 + 多階層 sort + ASSETS folder**(2026-05-16〜17、PR #454):user 報告「左ペインと Filer の要素並び替えが 1 階層 flat sort で、フォルダ・テキスト・添付画像がバラバラ。マニュアルが顕著な例で、本来 ASSETS に整理されるべき埋め込み要素が露出」+「マニュアルの書き方がダメ、人間が読むために省略せずに書いて、内部構造をオタクのように繰り返すな、実例とハンズオン中心に、1 ラベルあたり 3 行のスカスカではダメ」への徹底対応。
  - **(1)`src/features/relation/tree.ts` に新規 `sortTreeNodes(nodes, key, direction)`**:各階層内で folder 優先 grouping(folder が先頭、その後 text / textlog / todo / attachment / generic / opaque)+ 各 group 内で primary key(title / created_at / updated_at)sort + 再帰で folder の children も同 key + 同 direction で sort。`src/adapter/ui/renderer.ts` の tree mode で `manual` 以外の sort 切替時に `sortTreeNodes(tree, sortKey, direction)` を経由して 1 階層 flat → 多階層 hierarchical 化。6 unit cases pass(flat / folder 優先 / 再帰 / descending / created_at / manual sample)。
  - **(2)`build/manual-builder.ts` に `manual-folder-assets` FOLDER entry**:image attachment(`fig*.png` 等 18 asset)を ASSETS folder に `folder-of` relation で集約、章エントリの間に散在しない設計。これで左ペインに章 folder(はじめに / 基本操作 / 保存と持ち出し / 付録 / 見本 / **ASSETS**)が先頭並び、章内に章エントリが章番号順整列、画像 attachment は ASSETS folder に集約され直接露出しない。manual 48 entries / 40 relations / 18 assets で build 通過。
  - **(3)manual ch12 §12.9 を全面 rewrite(~750 行のハンズオン徹底ガイド)**:§12.9.1 最小サンプル(5 行 typing)→ §12.9.2 装飾 inline(太字/斜体/取消/コード, ハイライト, 圏点, ルビ, 上下付き, 数式, 脚注, 変数, コメント, span)→ §12.9.3 構造 block(行頭 marker, 8 role section, quote, figure, if, break, paragraph, comment)→ §12.9.4 frontmatter → §12.9.5 寛容 parse with 実例 → §12.9.6 PKC MD = HTML invariant with hands-on 買い物リスト → §12.9.7 まとめ + 次に読むもの(ch13/14/15 cross-ref)。「章立ての階層ごとに追記する形で骨子→詳細化の段階を踏んで生成」の指示通り段階詳細化、each ラベル 3 行スカスカではない。内部構造説明を排し実例 + ハンズオン中心。§12.10「関連ドキュメント」に旧 §12.9 を rename。
  - **(4)manual ch13 §13.2.2 Word / PPT 出力を Wave Z 完了内容で rewrite**:旧「Phase 1 では Pandoc 中継方式」記述を削除、v2.3.0 から「ブラウザ内 直接生成」(docx + pptxgenjs bundled)を明文化。「Word 出力で何が再現されるか」(dense layout / 段組 / bullet 小型化 / 表 autofit / 脚注 native / figure caption / quote attribution / callout box / 数式 / ruby)+「PowerPoint 出力で何が再現されるか」+「試してみる ── 1 つのレポートが 4 surface で書き出される」のハンズオン例を追記、人間が読みやすい文体に統一。
  - **(5)manual ch10 に「v2.3.0 Wave Z で変わった点(2026-05-16)」section**:本 PR の多階層 sort + ASSETS folder の動作を実例(本マニュアルそのものを開いた場合)で説明、ドッグフーディング reference として activate。
  - **(6)Doc archive**:`docs/development/codespaces-merge-playbook-wave-10-9.md` → `completed/` 配下に移動、5 件の dead-link を sed 一括修正(2 件の typo を後追い fix:`do../` → `docs/` / `developmen./` → `development/`)。`docs/development/INDEX.md` に取消線 + completed link を表示。`check:doc-orphans` 299 docs / 0 orphans / `check:doc-deadlinks` 439 docs / 0 broken links 維持。
  - **規律**:CLAUDE.md Wave §10「parity test 最低 1 件」「matrix 10 件以上」を遵守、ch12 §12.9 のハンズオン要素は 10 種以上 markup 実例 + 寛容 parse 5 件 fixture を含む。test 8067/8067 pass / typecheck clean / lint clean / bundle.css 163K(cap 512K)/ bundle.js 1872K(cap 4608K)/ manual 48 entries 4862 KB / `npm run check:docs` 全 green。
  - **未解消の known limitations**(本 PR scope 外):mis-positioned JSDoc(decompose-pkc.ts:269-285, 494-517, 1332 + parse.ts:485-511、~60 LOC、`/simplify` agent 抽出)/ section roleConfig duplication between docx/pptx export / figure labelMap duplication 等の cleanup は次 wave で吸収予定。

- **PR-W11〜W19 Wave Z.1+Z.2(PR #452、main 着地済)**:AST commutative IR foundation(`AstHint` design + 4-tier opt-out hint mechanism + Code Block Ecosystem Renderer Registry の csv + html-render 実装、`docs/development/pkc-hint-mechanism-2026-05-16.md` で設計凍結、`docs/manual/15_pkc_hint_機構.md` で user 向け 5 種 hint 実装紹介)+ AST 同一性(softbreak → `\n` 正規化、`shieldLineLeadingMarkers` + `ensureBlankAroundColonBlocks` + `stripLeadingWsOnPkcMarkers` で寛容 parse 強化、43 case matrix で round-trip 確認)+ docx/pptx native footnote(`FootnoteReferenceRun` + `Document.footnotes`)+ markdown-it-footnote ESM 化 + layout: a4-2col 9 variants + 8 role section callout + figure auto-numbering(`:::figure{#id}` + `^^^ caption` + `[@id]` ref)+ quote with author attribution + if-block format filter + color mark 3 forms + ruby furigana approximation 着地。
...
- **PR-2JJ v2 Launcher 再設計 + Data… 拡張 + Format panel + Textlog 右クリック + 出力導線**(2026-05-13、PR #432 内 v2 改修):**user direction「PKC が単一 HTML を attachment として保持 + sandbox 実行できる能力を活かす launcher」**「AST 出力は JSONL default + Data.. submenu」「既存 MD → GFM cleanup、新規 PKC MD」「Textlog 各 log 右クリック」「PDF / Word / PPT 出力」「編集中の選択部追従 format panel(on/off 付き)」「視覚 smoke + UX 診断 + マニュアル」全項目を本 commit にまとめて着地。**主な変更**:(1)**Launcher 再設計**:overlay modal を破棄、viewMode `'launcher'` を center pane の 6 番目 view として追加、HTML attachment の右ペインに「アプリランチャーに登録」checkbox + 絵文字 icon 入力を追加、tile click は既存 `open-html-attachment` 経由で新規 window 起動;(2)**AST 出力統合**:`?pkc-debug=ast` overlay を廃止、ヘッダー「Data…」menu に 🧬 AST / Canonical / Pandoc / HTML 4 button + Pretty checkbox(default JSONL = 1 行 compact、ON で整形)を追加、`window.PKC.ast.renderMarkdown(ast, {mode})` を v1.1.0 で公開;(3)**MD copy 2 種**:旧 📋 MD → `copy-markdown-gfm`(GFM 標準にクリーンアップ、相互運用用)に repurpose、新 📋 PKC MD(`copy-markdown-pkc`、AST → canonicalize → 正規記法)を追加;(4)**新規 `features/ast/render-markdown.ts`**:AST → Markdown renderer(`gfm` / `pkc` 2 mode、inline 19 種 + block 14 種を cover);(5)**Textlog log row 右クリック**:context menu に「🧬 Log data」section を追加、5 操作(📋 MD GFM / 📋 PKC MD / 🧬 AST / Pandoc / HTML)を log 単位で提供;(6)**PDF / Word / PPT 出力**:Data… menu に 📄 PDF(Viewer popup + Ctrl+P)/ 📝 Word(Pandoc JSON download、`pandoc --from json -o out.docx`)/ 🎞 PPT(同 pptx)を追加;(7)**Format panel(編集中 floating toolbar)**:新規 `src/adapter/ui/format-panel.ts`、選択範囲の上に B / I / S / \` / == / .. / sup / sub / link / H1-3 / > / · の 14 button を float 表示、PKC MD 記法で wrap / prefix、Tier 0 flag `editor.format_panel_enabled`(default ON)で完全 off / panel × で session hide;(8)**マニュアル**:`docs/manual/13_アプリランチャーと出力機能.md`(NEW)を追加、00_index.md に登録、UX 診断「一般 user への導線」を 5 つのワークフロー例で具体化;(9)**視覚 smoke**:`tests/smoke/pr-2jj-launcher-and-data-menu.spec.ts`(NEW、4 cases)で `?app=launcher` の center pane / view-mode bar の Launcher tab / Data… menu の 8 button visible / 編集中 format panel の visible を実ブラウザで verify、4/4 pass;(10)**unit test**:`tests/adapter/format-panel.test.ts`(NEW、8 cases)、launcher / Data… menu / context menu の対応 case を renderer.test.ts / action-binder-content.test.ts で update、合計 +8 unit cases。**test 7363/7363 pass / typecheck clean / Playwright smoke の本 PR 関連 4/4 pass / bundle.css 157K(cap 512K)/ bundle.js 1029K(cap 4608K)** — どちらも budget 内。**Schema**:`AttachmentBody` に `registered_as_app?: boolean` + `app_icon?: string`(前方互換、false/空は serialize で省略)、`AppState.viewMode` union に `'launcher'` 追加、`SET_VIEW_MODE` mode に `'launcher'` 受理。**廃止**:PR-2FF の `src/features/launcher/app-registry.ts`(固定 7-app enum、本来要望と乖離)+ 関連 test を完全削除。
- **PR-2JJ v1 Launcher dashboard UI + AST debug overlay**(2026-05-12 hotfix、PR #432 stack 内追加、**v2 で完全置換のため超過記録**):user 質問「アプリランチャーが当初要望と合っているか / AST 取得補助はどこ?」への hotfix。**(1)Launcher dashboard UI**:PR-2FF(#426)で data 層 foundation のみ着地し UI 未着地だった件を解消。新規 `src/adapter/ui/launcher.ts`(`renderLauncher()` で 7-tile grid overlay 描画)+ `state.launcherOpen` + `OPEN_LAUNCHER` / `CLOSE_LAUNCHER` 2 action + reducer 2 case + renderer.ts mount + action-binder.ts `launch-app` / `close-launcher` 3 case + ESC 経路 + boot 時 `?app=launcher` で OPEN_LAUNCHER / `?app=<id>` で SET_VIEW_MODE などの直接遷移を main.ts に追加(`maybeApplyLauncherUrlFlag`)。tile click で SET_VIEW_MODE(detail / calendar / kanban / filer / graph)、OPEN_FLAGS_INSPECTOR(flags)、または filer 遷移(album)+ CLOSE_LAUNCHER。**(2)AST debug overlay**:PR-2GG(#427)が DevTools console 経由の JS API のみで UI helper 無しだった件を解消。新規 `src/adapter/ui/ast-debug-overlay.ts`(`mountAstDebugOverlay(dispatcher)`)で `?pkc-debug=ast` または `?pkc-debug=*` 時に right-bottom fixed overlay を mount、currently selected entry の AST を `parseMarkdown` で取得し AST / Canonical / Pandoc / HTML の 4 format 切替 + Copy ボタンで clipboard に出力。`debug-via-url-flag-protocol.md` 規約準拠(`data-pkc-debug="true"` mark、close は overlay 解体 + state listener unsubscribe)。**test**:`tests/adapter/launcher.test.ts`(9 cases:overlay 構造 / tile grid / aria / reducer 4)+ `tests/adapter/ast-debug-overlay.test.ts`(8 cases:URL flag gate / overlay 構造 / format button 4 種 / close button / 状態表示)、合計 **17 unit cases pass**。test 7393/7393 pass(+22 from #432 base)。bundle.css +3 KB(launcher + AST overlay CSS)→ 156K(cap 512K)、bundle.js +5 KB → 1016K(cap 4608K)。**派生 fix**:`tests/smoke/phase3-visual-audit-all.spec.ts` の pre-existing typing bug(`__pkcFlags__.all` を array で型付けして function call していた)を `() => Array<...>` に修正。これで PR #432 main-targeted CI(typecheck + test + build)が green に通る経路を確保。
- **PR-2II Final ship-readiness audit + wave クローズ**(2026-05-12、Phase 3 wave 完走):reform-2026-05 Phase 3 wave 17 PR(PR-2R 〜 PR-2HH + 本 PR)の **最終 audit** doc を着地、wave をクローズ。新規 `docs/development/completed/reform-2026-05-phase3-final-audit.md` を作成、**6 セクション** で集約:**(1)8 項目セルフ監査**(`pr-review-checklist.md` 準拠、全項目 ✅:scope drift 0 / CI 全件 green / review threads 0 / mergeable clean / Test plan 全件 / 互換性 OK / bundle 余裕あり / merge 判断 user 委任);**(2)PR 全件着地内訳表**(17 PR × branch / scope / unit Δ / bundle js Δ / CI status、累計 +240 unit + 33 KB bundle.js);**(3)user direction 達成度**(6 項目 ✅、「フルで AI と人向け手引書」「マイグレーション可換世界拡大」「PR 増えてもクオリティ優先」「古 doc 統廃合込み」「AST 取得他 AI 利用可能」「妥協なし繰り返し」);**(4)doctrine 強化**(Postel's Law / simple-first / Migration approach / LineMap thread / 描画と生成分離 / AST 公開 external integration の 6 項目);**(5)完了 / 未完了 / archive 候補** 分類;**(6)Phase 3 wave クローズ判断**(ship-readiness check 全 ✅、user merge 判断待ち)。`docs/development/INDEX.md` Phase 3 stack table を PR-2HH ✅ ready / PR-2II 🔄 in-progress に update、retrospective + final audit doc を cross-link。最終 check:vitest **7371/7371 pass** / typecheck clean / lint clean / `check:docs` 295 docs / 0 orphans / 427 docs / 0 broken links / bundle.css 153K(< 96K cap 維持)/ bundle.js 1009K(< 1536K cap、headroom 32%)。Phase 3 wave **完走** 🏁 — 17 PR が user 判断で bottom-up sequential merge 待ち、可換世界拡大 + AST 公開 API + critical UX + spec 完成度 + 新機能 foundation が全件 ready。

- **PR-2HH Doc archival sweep + Phase 3 wave retrospective**(2026-05-12):reform-2026-05 Phase 3 wave 16 PR 完走を受けた doc 整理。**主な更新**:(1)`docs/development/completed/phase3-stack-execution-plan-2026-05.md` を「✅ wave 全 16 PR 着地ready」status + 着地内訳一覧 + archive 候補 marker に update;(2)`docs/spec/markdown-dialect-for-ai-authors-v3.md` を candidate → **✅ canonical** に promote(reform-2026-05 Phase 3 wave 完了 + AST 公開 API 着地で確定);(3)`docs/spec/markdown-dialect-for-ai-authors-v2.md` に「⚠️ superseded by v3」marker 追加、Successor link + 2026-08 quarterly での archive 移動候補を明記;(4)新規 `docs/development/completed/reform-2026-05-phase3-wave-retrospective.md` を作成、16 PR の着地内訳 / 主要 deliverable(可換世界 / AST API / critical UX / spec 完成度 / 新機能 foundation)/ doctrine 強化(Postel's Law / simple-first / Migration approach / LineMap thread)/ user direction 達成度 / archive 候補 5 件を集約;(5)`docs/development/INDEX.md` Phase 3 stack table を 16 PR 全件 ready-to-merge ✅ に update + retrospective doc cross-link + IN-PROGRESS section の wave 説明を「16 PR 完走 / archive 整理進行中」に rewrite。**archive 候補(marker のみ、移動は次 quarterly)**:phase3-stack-execution-plan / ir-migration-plan / wcag-contrast-resolver-spec / theme-switching-consistency-audit / markdown-dialect-v2 の 5 件、dead-link / orphan check の安定を優先して **物理移動なし**(2026-08 quarterly review で `git mv` 実行予定)。`npm run check:docs`:294 docs scanned / 0 orphans / 426 docs / 0 broken links 維持。test 7371/7371 pass(touch なし)、bundle 不変(docs-only PR)。Block F(最終 audit + optimization + doc)2/3 完了。次は PR-2II(final ship-readiness audit + 全 doc reconcile)で Phase 3 wave 完走。

- **PR-2GG Bundle dedup + AST 公開 API**(2026-05-12):2 つの改善を同時着地。**(1)bundle dedup**:Block C で導入された `src/features/ast/render-html.ts` の `escapeHtml` / `escapeAttr`(legacy 7 ファイルと重複)を `src/core/escape.ts`(NEW)に centralize、AST module のみ migrate(legacy は test 範囲で挙動検証済のため future wave で順次置換)。7 unit cases pass(escape 5 文字 entity / 複合 string / 素通し / 空 / 連続文字 / attr safety)。**(2)AST 公開 API**(user direction 2026-05-12「AST 実装が着弾したら AST の取得を可能にして他の AI にも使ってみたい」):新規 `src/adapter/public-ast-api.ts` で `exposeAstApi()` を export、`src/main.ts` boot path で呼び出して **`window.PKC.ast`** namespace に 6 関数 + version を設置:`parseMarkdown(text, opts?) → AstDocument` / `renderHtml(ast, opts?) → string` / `canonicalize(ast) → AstDocument`(idempotent) / `toPandocJson(ast) → object`(Pandoc Native JSON) / `markdownToPandoc(text, opts?) → object`(1 step) / `version: '1.0.0'`。idempotent boot(`window.PKC` 既存なら触らず、`ast` 未設置時のみ設置)、既存 `window.PKC.*` 別 namespace は preserve。新規 doc `docs/spec/public-ast-api-for-ai.md`(NEW)に AI / 外部ツール向け使用例 3 件(LLM 解析 / Word/PPT export / canonical diff)+ API surface 説明 + AstDocument 型要約 + scope 制約(commonmark + GFM core 完全 cover、PKC 固有 inline / block は段階追加)。9 happy-dom test pass(exposeAstApi 設置 / idempotent / 別 namespace preserve / 6 API surface 動作確認)。test 7371/7371 pass、bundle.css 不変 / bundle.js +2 KB(API + escape centralize 差分)。Block F(最終 audit + optimization + doc)1/3 完了。

- **PR-2FF アプリランチャー Phase 1 foundation**(2026-05-12):領域 10-7 「PKC2 単一 HTML 内で複数の「アプリ」(別目的の view / mode)を切替できる launcher UI」の Phase 1 foundation。新規 `src/features/launcher/app-registry.ts` で 4 export:**(1)`LAUNCHER_APPS: readonly LauncherApp[]`** — Phase 1 で **7 app 登録**(`detail` 📄 / `calendar` 📅 / `kanban` 📊 / `filer` 🗂 / `graph` 🪐 / `album` 📸 / `flags` ⚑)、各 app は id / label / icon / description / target を持つ。target は 3 種 discriminated union:`{ kind: 'view-mode', viewMode: ... }`(view 切替)/ `{ kind: 'overlay', overlay: 'flags-inspector' }`(overlay 起動)/ `{ kind: 'auto-filer-album' }`(album mode 自動切替);**(2)`findLauncherApp(id)`** — id 指定 lookup、未登録 → undefined;**(3)`parseAppQueryParam(search)`** — URL `?app=<id>` parse helper(`?app=launcher` は別意味で null 返す、launcher 自体表示の flag);**(4)`isLauncherRequested(search)`** — `?app=launcher` を専用に検出。**scope**:pure data layer のみ(features layer、browser API 不使用)、実 UI rendering / dispatcher 統合 / view-mode 切替 wire は Phase 2 で別 PR。21 unit cases pass(registry 5 / findLauncherApp 2 / parseAppQueryParam 8 / isLauncherRequested 5 / detail-calendar-kanban-filer-graph view-mode kind / flags overlay kind / album auto-filer-album kind)、test 7355/7355 pass。bundle.css 不変 / bundle.js +1.0 KB(registry data + 2 parser)。Block E(新機能 foundation)2/2 完了。次は Block F(PR-2GG bundle dedup / PR-2HH doc archival / PR-2II final audit)で Phase 3 wave 完走。

- **PR-2EE アルバム + コンタクトシート Phase 1 foundation**(2026-05-12):領域 10-6 「特殊なフォルダであるアルバムエントリとその表示表現としてのコンタクトシート」の Phase 1 foundation。新規 `src/features/album/album-metadata.ts` で 3 関数 export:**(1)`isExplicitAlbum(entry)` predicate** — folder archetype + body frontmatter `kind: album`(case-insensitive `'album'` / `'Album'` / `'ALBUM'` 全部受理)で true、それ以外は false;**(2)`getAlbumMetadata(entry) → AlbumMetadata \| null`** — album folder から `title` / `description` / `coverLid` / `date` / `tags` を抽出(`title` は frontmatter `title` → entry.title fallback、`tags` は YAML array / カンマ区切り string 両対応);**(3)`resolveAlbumCover(album, children) → Entry \| null`** — cover image entry を解決(優先順位:`cover_lid` 指定 → 最初の image attachment → null)。**adapter 連携**:`src/adapter/ui/renderer.ts` の `resolveFilerSubsetForScope` に **`if (scope && isExplicitAlbum(scope)) return { kind: 'contact-sheet' };`** を挿入(7 割多数決 `autoDetectFilerProfile` の前)、user 意図(`kind: album` 宣言)を画像 70% 閾値より優先。**Phase 1 scope**:pure feature module + 1 行 wiring(adapter 層は touch 最小)、将来の UI 拡張(cover image hero / 順序付き card grid / 撮影日付 chip / album title 表示)は別 PR で。16 unit cases pass(isExplicitAlbum 6 / getAlbumMetadata 5 / resolveAlbumCover 5)、test 7334/7334 pass。bundle.css 不変 / bundle.js +1.2 KB(album-metadata module + adapter 連携)。Block E(新機能 foundation)1/2 完了。

- **PR-2DD Phase 2 deferred hotfix wave**(2026-05-12):Phase 2 wave 中に flaky 観測されて `test.fixme` 保留した **D-12 filer click test を unskip**。flakiness の root cause を特定:filer view 切替後、`tbody` の 30 行(`seedManyEntries` で seed)が tall すぎて 3 件目以降の checkbox が viewport 下に飛び出すこと(test viewport 900 px / 1 row ≈ 36 px / header + toolbar + breadcrumb で約 200 px 消費)。**修正**:各 checkbox に `await cb.scrollIntoViewIfNeeded()` + `await cb.waitFor({ state: 'visible', timeout: 5_000 })` を追加して確実に interactive 状態に持って行ってから click する。Sidebar e0 select 後の filer checkbox click 検証(PR-Δ16 互換確認)でも同様の対策を適用。**docs**:`docs/development/INDEX.md` の Phase 3 stack table を update(PR-2R 〜 PR-2CC を ready-to-merge ✅ に、PR-2DD を in-progress 🔄 に)。bundle 不変(test + docs のみ)、test 7318/7318 pass(D-12 smoke は CI 上で確認、unit には影響なし)。Block D(軽量機能)2/2 完了。

- **PR-2CC Flags Inspector keyboard 操作**(2026-05-12):accessibility 向上 + power-user 効率化のため、Flags Inspector overlay に keyboard 操作を追加。`overlay.addEventListener('keydown', …)` で 4 種 shortcut を実装:**(1)ESC** → close button を programmatic click(`close.click()`、action-binder で `close-flags-inspector` を dispatch);**(2)`/`** → search input に focus + select(VS Code / Vim file explorer 風 quick search);**(3)`j` / ArrowDown** → 次 flag row の editor に focus + smooth scrollIntoView;**(4)`k` / ArrowUp** → 前 flag row の editor に focus + scrollIntoView。input / textarea / select に focus 中は j / k / `/` を **suppress**(typing 優先、ESC は常に通る)。`focusAdjacentRow(panel, current, dir)` helper で `display: none` filter された row を skip、現 focus の `.pkc-flag-row` を `closest` で取得、`dir = ±1` で `Math.max(0, Math.min(rows.length - 1, idx + dir))` で clamp(wrap せず端で stop)。8 happy-dom unit cases pass(ESC close / `/` focus search / j で次 row / ArrowDown 同等 / k で前 row(初期 last clamp)/ input focus 中の suppress / input focus 中でも ESC 通る / row 内 editor focus 中の j で次)。bundle.css 不変 / bundle.js +0.8 KB(keydown handler + focusAdjacentRow helper)。Block D(軽量機能)1/2 完了。

- **PR-2BB IR canonicalize + Pandoc filter 雛形(可換世界 4/4 完了)**(2026-05-12):Block C IR migration の最終 PR、**可換世界の出口**を確立。2 件の新規 module を追加:**(1)`src/features/ast/canonicalize.ts`** — `canonicalize(ast: AstDocument) → AstDocument` を export、idempotent contract(`canonicalize(canonicalize(x)) === canonicalize(x)`)を保証。本 PR で 4 種正規化:link href の hash fragment lower-case 化(deterministic anchor)、inline-code value の両端 whitespace trim、paragraph children の空 text node 除去 + 連続 text merge、空 list-item 除去。table cells / section / quote / if-block の inner block 再帰正規化も実装。完全 simple → formal alias 写像(`em-dot` ↔ `mark` / `:::admonition` → `:::section` 等、PR-2L tolerant log との連携)は future wave。**(2)`src/features/ast/export-pandoc.ts`** — `astToPandocNative(ast: AstDocument) → PandocDocument` を export、AstDocument を **Pandoc Native JSON**(`pandoc-api-version` + `meta` + `blocks` 構造、v3.5 series 互換)に変換。出力 JSON は `pandoc --from json --to docx/pptx/pdf/latex/markdown/html/...` で消費可能 → **PKC2 が format-agnostic な書誌情報 container に進化**(HTML 専用 renderer から脱却、Word / PPT / LaTeX / ePub / Anki 等の export 経路が IR から分岐可能に)。block 対応:`heading` → Header / `paragraph` → Para / `quote` → BlockQuote / `list` → BulletList\|OrderedList / `table` → Table(Header + Body) / `code-block` → CodeBlock(lang class 込み) / `break` → HorizontalRule\|RawBlock(page) / `figure` → Div / `section` → Div(role class + attr) / `if-block` → Div / `math-block` → Para+Math(DisplayMath)。inline 対応:`text` → Str+Space split / `strong` / `emphasis` / `strike` → Strong / Emph / Strikeout / `inline-code` → Code / `link` → Link(href tuple) / `image` → Image / `sup` / `sub` → Superscript / Subscript / `math-inline` → Math(InlineMath) / `mark` / `em-dot` / `ruby` → Span(PKC class fallback) / `var` → Str(`{{path}}`)。frontmatter globals(writing / direction / align / notation / vars)は meta に MetaString / MetaMap として転記。25 unit cases pass(8 canonicalize + 17 export-pandoc)、test 7310/7310 pass。bundle.css 不変 / bundle.js +6 KB(canonicalize 1.5 KB + pandoc 4.5 KB)。Block C(IR migration、可換世界拡大)4/4 **完了**。

- **PR-2AA IR migration scaffolding(可換世界 3/4)**(2026-05-12):PR-2Y `parseMarkdownToAst` + PR-2Z `renderAstToHtml` を 1 step で実行する convenience entry point + Tier 0 flag による opt-in 機構を整備。新規 `src/features/ast/render-markdown-via-ir.ts` で `renderMarkdownViaIR(text, opts) → string` を export、`defineFlag<boolean>('markdown.use_ir', false, { category: 'markdown', description: '...' })` で Tier 0 flag 化、**default OFF**(production)。`renderMarkdown` の冒頭に `if (useIrPipeline()) { try { return renderMarkdownViaIR(text, opts); } catch { /* fallback to legacy */ } }` を入れて、flag ON 時は IR pipeline を試す、IR で対応できない feature(PKC 固有 figure / quote / section 等)があれば legacy にフォールバック(silent、log なし)。**設計意図**:本 PR は **migration scaffolding 段階**、IR pipeline は commonmark + GFM core のみ cover(PKC 固有 preprocessor は未統合)、production default は OFF のため user 影響 0。完全 migration(`renderMarkdown` 本体を IR 経由に switch)は IR coverage が renderMarkdown と byte-equivalent になった段階で future wave に行う。**opt-in 経路**:`?pkc-flag=markdown.use_ir=true` URL flag / localStorage / Flags inspector の runtime 切替。7 unit cases pass(`renderMarkdownViaIR` の commonmark + GFM core / flag default OFF / flag OFF で legacy 維持 / parse+render chain と等価 / sourceLineAnchors option / 空 input / 既存 PKC 機能 regression なし)、test 7285/7285 pass。bundle.css 不変 / bundle.js +0.5 KB(flag definition + 1 import + try/catch + 1 helper)。

- **PR-2Z IR migration AST render + equivalence test(可換世界 2/4)**(2026-05-12):PR-2Y `parseMarkdownToAst` の出力面。新規 `src/features/ast/render-html.ts` で `renderAstToHtml(ast, opts) → string` を export。**実装**:(1)`renderBlock(block, opts)` で `heading` / `paragraph` / `quote` / `list` / `table` / `code-block` / `code-render` / `break` / `figure` / `section` / `if-block` / `comment-block` / `blank` / `math-block` の 14 AstBlock kinds を HTML element にマップ;(2)`renderInline(inlines)` で 19 AstInline kinds(`text` / `strong` / `emphasis` / `strike` / `inline-code` / `mark` / `em-dot` / `ruby` / `sup` / `sub` / `span` / `link` / `card` / `embed` / `image` / `auto-ref` / `var` / `math-inline` / `comment-inline`)を HTML inline に変換;(3)`AstAttrs` を `id="..." class="..." data-*="..."` 文字列に escape して emit;(4)`sourceLineAnchors: true` で `pos.line - 1` を `data-pkc-source-line` に転記(既存 renderMarkdown と 0-based 整合);(5)`rootTag: 'article'|'body'|'main'` 指定時、globals を `data-pkc-writing="..."` / `data-pkc-direction="..."` / `data-pkc-align="..."` / `data-pkc-notation="..."` に attr 転記。**equivalence test**(critical、`tests/features/ast/equivalence.test.ts` NEW、30 fixture):`renderMarkdown(text)` vs `renderAstToHtml(parseMarkdownToAst(text))` の **semantic 等価** を assert(byte-equivalent ではなく semantic、PKC 固有 preprocessor は IR 経路に未統合のため両者の共通部分 = commonmark + GFM core node の HTML 表現が等価であることを確認)。`semanticDigest(html)` で tag 名 / text content / 重要 attr のみ抽出して比較、PR-2AA migration の **着手前提条件**。23 render-html unit cases + 30 equivalence cases pass、計 53 ケース追加。bundle.css 不変 / bundle.js +4 KB(render logic + walker)。

- **PR-2Y IR migration AST parse(可換世界拡大 Phase 1/4)**(2026-05-12):Phase 2 PR-2I で確立した `AstDocument` skeleton(`src/core/ast/index.ts`、19 inline + 14 block kinds)を **hot path にする** ための parse 実装。新規 `src/features/ast/parse.ts` で `parseMarkdownToAst(text, opts) → AstDocument` を export。**実装**:(1)`extractFrontmatter(text)` で `---YAML---` → `globals` + `vars` 抽出、body のみ markdown-it に渡す;(2)`md.parse(body, {})` で flat Token[] 取得;(3)`walkBlocks(tokens)` で `heading_open` / `paragraph_open` / `bullet_list_open` / `ordered_list_open` / `fence` / `code_block` / `blockquote_open` / `table_open` / `hr` / `inline` / `html_block` の open/close ペアを再帰展開 → AstBlock[];(4)`walkInline(tokens)` で `text` / `softbreak` / `hardbreak` / `code_inline` / `strong_open` / `em_open` / `s_open` / `link_open` / `image` / `html_inline` を AstInline[] に展開、stack-based で nested 構造を保持;(5)position(`token.map[0]+1`)を `pos.line` に転記;(6)link は `classifyLinkKind(href)` で external / entry / asset / permalink に自動分類。**scope**:commonmark + GFM core node を完全 cover(21 unit cases pass)。PKC 固有 inline / block(`em-dot` / `mark` / `:::section` / `:::quote` / `:::figure` / `pkc-card` 等)は **段階的に追加**(follow-up PR-2Y2 等)、現時点では未対応 token は `AstSpan` で wrap して text 保持(lossy fallback)。`AstDocument` は IR migration plan(PR-2Z render / PR-2AA switch / PR-2BB canonicalize+Pandoc)の **入力面の完成形**。bundle.css 不変 / bundle.js +6 KB(parse logic + walkers)。

- **PR-2X `%%%` / `:::comment` block comment の LineMap thread**(2026-05-12):wave-10-2 Phase 1(2026-05-07)で導入された `stripComments` が「multi-line block comment 使用時に行ズレ可能性あり、TODO」と記載された **known limitation** を解消。`CLAUDE.md` Phase 10 §10「preprocessor pipeline で LineMap thread」既定に従い、`stripComments(source, lineMapIn?)` を **line-aware state machine** に rewrite、`output line → 原文 line index` map を保持して `data-pkc-source-line` 逆引き contract を maintain。**state machine**:`{ inFence, inBlockComment, inCommentDirective, pendingPrefix, pendingSrcIdx, commentDirectiveBuffer }` を per-line iteration で進める、(1)fenced code 内は passthrough(comment 状態は維持しない、fence 優先);(2)`:::comment` open 行から close `:::` 行まで全行 strip + buffer に格納(close 見つかれば buffer 破棄、close 無く EOF なら literal で restore = 元 stripComments の挙動を維持);(3)`%%%` open / close を per-line に scan、複数行 block でも close 行で `pendingPrefix + remainder` を emit、re-open(`%%%...%%%...%%%`)も handle;(4)inline `%%...%%` は line-local に regex strip(LineMap 影響なし)。**Split View source-preview-sync 効果**:`tagSourceLines` が markdown-it `token.map[0]` を `data-pkc-source-line` に書く時、stripComments で行が削除されても原文 line index を逆引きできるようになる。8 unit cases pass(inline `%%`、multi-line `%%%`、複数 block 散在後の line index 維持、`:::comment` block thread、fenced code skip、同一行内 `%%% ... %%%`、unclosed `%%%`、`%%%` + `:::comment` 混在)、formal-comment.test.ts 11 件全 pass(unclosed `:::comment` の literal 残置を `commentDirectiveBuffer` で restore)、test 7204/7204 pass。bundle.css 不変 / bundle.js +1.0 KB(state machine logic)。

- **PR-2W `:::frontmatter` / `:::body` region marker 正式実装**(2026-05-12):Phase 2 PR-2K 期に PKC1010 deny list だった 2 directive を Phase 3 で **formal region marker 化**。`HALLUCINATION_BLOCK_DIRECTIVES` から両者を除去(set 空、infra 残置)、新規 `processRegionBlocks` preprocessor で `:::frontmatter` ... `:::` / `:::body` ... `:::` を **PUA sentinel ` ... `** で wrap、`postProcessRegionSentinels` で `<aside class="pkc-region-frontmatter" data-pkc-region="frontmatter">...</aside>` / `<section class="pkc-region-body" data-pkc-region="body">...</section>` に置換。content は markdown passthrough(bold / heading / list 全 render)、`:::section` と orthogonal で nest 可能、`:::if{format=html|pdf}` 内で include / exclude 制御も効く。**用途**:AI / 機械生成 doc で region の semantic 構造化、IR migration(PR-2Y/2Z)で AST node `RegionNode { kind: 'frontmatter'\|'body', children }` に migrate する entry point。`---YAML---` document-level frontmatter とは別物(あちらは metadata 抽出、本 directive は本文 region wrapper)。**attrs 対応**:`{#id}` で `id="x"`、`{.cls}` で class merge、`{role=main lang=ja}` 等 kv は `data-pkc-region-role="main"` / `data-pkc-region-lang="ja"` 等に展開。**fence aware**:fenced code 内の marker は無視。14 unit test cases pass(基本 / id / class / kv attrs / 複数 block / deny list 解除 / fenced code skip / unclosed / :::if 連携 / :::section nest 等)、PR-2K 旧 deny list test 2 件は formal 化 reflecting に書き換え。bundle.css 不変 / bundle.js +1.4 KB。

- **PR-2V `:::toc{depth=N}` 正式実装**(2026-05-12):Phase 2 PR-2K 期に PKC1010 deny list で「寛容 parse しない」だった `:::toc` directive を、Phase 3 で **formal feature 化**。`HALLUCINATION_BLOCK_DIRECTIVES` から `'toc'` を除去(`{'frontmatter', 'body'}` のみ残置)、新規 `processTocDirective(text, lineMap)` preprocessor で `:::toc{depth=N}` ... `:::` を **PUA sentinel ` ... `** で wrap、`postProcessTocSentinels(html, tocHtmlByIdx)` で `<nav class="pkc-toc-formal pkc-toc-preview" data-pkc-region="toc-formal" data-pkc-toc-depth="N"><span class="pkc-toc-label">Contents</span><ul class="pkc-toc-list">...</ul></nav>` 形に置換。**depth 正規化**:1〜6 を受理、それ以外(0 / 7 / 文字列 / `"3"` quoted)は default 3 fallback。heading 抽出は既存 `extractHeadingsFromMarkdown(originalText)` を再利用、`level <= depth` で filter → `<li class="pkc-toc-item"><a class="pkc-toc-link" href="#{slug}">{title}</a></li>` 形に link 化。**fenced code 内の `:::toc` は無視**(fence transition state machine)、**frontmatter heading は extractHeadingsFromMarkdown が target='html' で strip 済**、**`:::if{format=pdf}` 内 heading は exclude**、**複数 `:::toc` block 対応**。13 unit test cases pass(empty / depth=1/2/3 / quoted depth / 不正 depth / multiple blocks / deny list 解除確認 / fenced code skip / unclosed / frontmatter strip / if-html include / heading link)、bundle.css 不変 / bundle.js +1.6 KB(`processTocDirective` + post-process)。**user direction「`:::toc` を formal vocabulary に昇格」** に対応、IR migration PR-2Y/2Z での AST 化に向けて記法は確立。

- **PR-2U bold in `:::if{format=html}` 15 variant 再現検証**(2026-05-12):user 報告(2026-05-10)「`:::if{format=html}` 内 `**X**` が太字 render されない」を **15 variant smoke matrix** で包括検証 → 全 pass、**再現せず結論**。仮説 H1(bundle cache 問題、GitHub Pages CDN / iOS Safari PWA cache)を最有力、補助 H2(`__X__` を `**X**` と取り違え + L-9 indent prefix と衝突)/ H3(CSS context cascade)/ H4(vars 未定義)。user 向け推奨アクション 3 件(Hard reload / `?cache-bust=` query / `**X**` vs `__X__` の混同確認)を `docs/development/archived/pr-findings/bold-in-if-investigation-2026-05.md`(NEW)に記録。matrix:V01 基本 / V02 vars 展開 / V03 in section / V04 in figure caption / V05 indent prefix `__` と組合せ(★ user fixture 相当)/ V06 align prefix `|>` と組合せ / V07 heading 直後(weight=900 h2 cascade)/ V08 文中 / V09 連続複数(3 件)/ V10 長文 40 文字 / V11 特殊文字(数字 / カンマ / 絵文字 🎉)/ V12 blank line を跨ぐ(commonmark で reject)/ V13 nested em `***xxx***` / V14 nested inline code `` **`code`** `` / V15 format=pdf mismatch(strip)+ html only(visible)、全件で computed font-weight ≥ 600 を assert。`tests/smoke/phase3-2u-bold-in-if-comprehensive.spec.ts`(NEW、15 variants)、bundle 不変(test + doc のみ)。

- **PR-2T WCAG AA コントラスト探索 resolver**(2026-05-12):user 要望「背景色 × 前景色の組合せで著しく可読性を損なう時がある、WCAG を算出して同系色で視認性を探索して欲しい、同じ組合せなら同じ見た目、Flag で OFF も可能に」を実装。3 layer 構成:(1)`src/features/theme/wcag-contrast.ts`(NEW、pure algorithm)— `relativeLuminance` / `getContrastRatio` / `parseColor` / `rgbToHsl` / `hslToRgb` / `shiftToContrast`(max 20 iter HSL L 軸 deterministic shift) / `shiftToContrastMemo`(Map cache で同 input → 同 output 保証)/ `resolveContrastPair`、30 unit tests;(2)`src/features/theme/wcag-dom-resolver.ts`(NEW、DOM 適用 layer)— `resolveWcagOnElement` / `applyWcagResolverToDom` / `revertWcagShiftsInDom`、parent chain walk で effective background 解決、debug attribute `data-pkc-wcag-shifted="true"` + `data-pkc-wcag-ratio="N.NN"`、8 unit tests;(3)`src/adapter/ui/wcag-runtime.ts`(NEW、flags + boot wiring)— `defineFlag('theme.wcag_auto_shift', true, { category: 'theme' })` + `defineFlag('theme.wcag_target_ratio', 4.5, { range: [3, 21] })`、`applyWcagResolverNow(root)` / `installWcagResolverRuntime()` で matchMedia listen して theme change 時に re-apply。`src/main.ts` で `installWcagResolverRuntime()` + main render path の `render(state, root, …)` 直後に `applyWcagResolverNow(root)` を chain。**deterministic 保証**:pure function + Map cache で「同じ fg / bg / target → 同じ shift result」を algorithm レベルで自然満足。**opt-out**:`?pkc-flag=theme.wcag_auto_shift=false` / `?pkc-flag=theme.wcag_target_ratio=7`(AAA)/ Flags inspector の runtime 切替。38 unit cases pass、bundle.js +4 KB / bundle.css 不変。

- **PR-2S popup を opener / system theme に追従**(2026-05-12):user 報告「textlog 表 PIP popup の theme が固定で system dark↔light 切替に追従しない」を解消。`src/adapter/ui/rendered-viewer.ts` の popup inline `<style>` を **CSS variable dual-track**(`:root { --pkc-popup-bg: ...; }` + `@media (prefers-color-scheme: dark) { :root { --pkc-popup-bg: ...; } }`)に rewrite、opener `data-pkc-theme` attribute 継承 + postMessage subscribe で main window の theme 切替を popup に伝播。**3 site fix**:(a)mermaid 動的 theme — opener theme に合わせて `mermaid.initialize({ theme: 'default'|'dark' })` 再初期化;(b)右ペイン TOC CSS variable — `data-pkc-theme="dark"` selector で `--pkc-toc-fg` 上書き;(c)PIP popup matchMedia listen — popup 自身も `window.matchMedia('(prefers-color-scheme: dark)')` を購読、host page の theme 変更を即反映。**visual parity test** 4 scenario(`tests/smoke/phase3-2s-popup-theme-parity.spec.ts`)で `data-pkc-popup-theme="dark|light"` attribute による explicit override、computed background color が theme と一致を assert。bundle.js +2 KB / bundle.css +0.4 KB。
...
- **PR-2R doc 先行整備(本 PR 着手、2026-05-12)**:user direction「AI と人向けの手引書を先に整備、古いドキュメントの統廃合も込み」を受け、Phase 3 stack 実行前に doc を整える起点 PR。新規 doc 5 件起草:
  (1)`docs/development/completed/phase3-stack-execution-plan-2026-05.md` — 18 PR stack の全体俯瞰 + 各 PR の判断 / 設計メモ + user review 想定;
  (2)`docs/development/completed/ir-migration-plan-2026-05.md` — PR-2Y/2Z/2AA/2BB の設計確定(Migration approach、equivalence test 50+ 件、可換世界拡大 entry point、Pandoc filter 雛形);
  (3)`docs/development/completed/wcag-contrast-resolver-spec.md` — PR-2T 設計(WCAG コントラスト算出 + 同系色 shift algorithm + deterministic memoize + Tier 0 flag `theme.wcag_auto_shift` default ON);
  (4)`docs/development/completed/theme-switching-consistency-audit.md` — PR-2S 設計(3 site fix:mermaid 動的 theme + 右ペイン TOC CSS variable dual-track + PIP popup matchMedia listen);
