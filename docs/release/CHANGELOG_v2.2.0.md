...
- **🚨 YAML natural extension follow-up:右ペイン TOC vars 展開 + Properties nested object 表示(2026-05-09)**:user 実機テスト報告「右ペインと Viewer 表示の TOC がアンカー文字列のままです」(screenshot:`{{vars.title}}` literal 表示 + `vars [object Object]` 表示)への対応。**問題**:(1) `extractTocFromEntry` の TEXT / TEXTLOG 経路が `entry.body` を直接 walk して heading 行から `{{vars.title}}` を literal で抽出、frontmatter strip + vars 展開を経由していなかった。(2) `formatFrontmatterValue`(renderer.ts)が nested object に到達したら fallthrough で `String(v)` → `[object Object]` を返していた(YAML extension で nested mapping を返すようになったため新たに発生)。**Fix**:(1) `extractTocFromEntry` の text / textlog 経路で `parseFrontmatter` で strip → 残り body を `extractHeadingsFromMarkdown` に渡す + 抽出後の text + log label を `expandVarsInTocText(text, vars)` で展開。textlog の log でも per-log frontmatter / vars に対応。3 surface(center pane 右ペイン / Viewer popup TOC / entry-window)全部に効果。(2) `formatFrontmatterValue` で nested object を `{ k: v, … }` の compact 表現に正規化(深さは parser の `maxDepth=4` で抑えられているので JSON 化で過大にならない)。**テスト**:unit `+5` 件(`markdown-toc.test.ts`:nested vars / flat dot vars / 未定義残置 / frontmatter strip / regression)+ visual parity smoke 1 件(`wave-10-2-phase2-yaml-toc-properties-fix-parity.spec.ts` NEW、user の screenshot シナリオを実機 reproduce、右ペイン Contents 3 行とも `親` + Properties `{ title: 親 }` + Viewer popup TOC 同一 + screenshot 2 枚)。全 6799 unit pass、wave-10-2 phase 2 smoke 6/6 pass。**bundle 影響**:bundle.js +0.4 KB / bundle.css 不変。
