...
- **Card widget の高度機能 未実装** — `@[card](entry:<lid>)` の parser + renderer placeholder + click/keyboard wiring(Slice 1-4)+ minimal chrome(archetype badge + entry title + missing/cross-container/malformed 状態、Slice 5.0、`adapter/ui/card-hydrator.ts`)が 2026-04-25 までに着地。**残るのは excerpt(Slice 5.1)、thumbnail(Slice 5.2)、advanced variants(`compact` / `wide` / `timeline`、Slice 6)のみ**(将来 slice で再評価)。Embed presentation のうち `![](entry:...)` transclusion は引き続き稼働中
