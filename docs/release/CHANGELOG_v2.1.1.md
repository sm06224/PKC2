...
- **Card widget の高度機能 未実装** — `@[card](entry:<lid>)` の parser + renderer placeholder + click/keyboard wiring(Slice 1-4)+ minimal chrome(archetype badge + entry title + missing/cross-container/malformed 状態、Slice 5.0、`adapter/ui/card-hydrator.ts`)+ excerpt(plain-text、TEXT / TEXTLOG / TODO / FORM / FOLDER の archetype 別 source、Slice 5.1、`features/card/excerpt-builder.ts`、XSS 回避のため markdown-it / asset-resolver 経路を一切経由しない pure helper)が 2026-04-25 までに着地。**残るのは thumbnail(Slice 5.2)、advanced variants(`compact` / `wide` / `timeline`、Slice 6)のみ**(将来 slice で再評価)。Embed presentation のうち `![](entry:...)` transclusion は引き続き稼働中
