...
## 3. Category A — 確定 dead（削除推奨）

### 3.1 `isMarkdownEvaluatedArchetype(archetype)`
...
本書を根拠に、以下の最小 patch を follow-up PR で適用することを推奨:

| ファイル | 変更 |
|---|---|
| `src/features/connectedness/sets.ts` | `export function isMarkdownEvaluatedArchetype` 行の `export` を削除。関数本体は削除（内部でも呼ばれていないため） |
| `src/features/connectedness/index.ts` | barrel から `isMarkdownEvaluatedArchetype` を除去 |
| `tests/features/connectedness/sets.test.ts` | `describe('isMarkdownEvaluatedArchetype')` ブロック（+ import）削除。本関数のテストは `MARKDOWN_EVALUATED` 定数の挙動を `buildConnectednessSets` の振る舞いとして間接的に検証する form に統合可能 |
| `src/runtime/contract.ts` | `export type SlotId` 行削除 |
| `docs/development/connectedness-s3-v1.md` | 「`isMarkdownEvaluatedArchetype` は v3 実装で inline 化、S3 export は v3 cleanup で削除」旨の追記 1 行 |

想定サイズ: src/ ~10 行削除、tests -20 行程度、docs +1 行。型チェック・lint・テスト・build 影響なし。
