...
|------|--------------|---------------|
| `image` | `image/png`, `image/jpeg`, `image/gif`, `image/webp`, `image/svg+xml`, `image/bmp`, `image/x-icon` | Data URI → `<img>` |
| `pdf` | `application/pdf` | Blob URL → `<object>` + 別窓ボタン |
| `video` | `video/*` | Blob URL → `<video controls>` |
| `audio` | `audio/*` | Blob URL → `<audio controls>` |
| `html` | `text/html`, `application/xhtml+xml` | Blob URL → sandboxed `<iframe>` + 別窓ボタン |
| `none` | その他すべて | ダウンロードのみ |

### 判定関数

- `isPreviewableImage(mime)` — 7 種の画像形式
- `isPreviewableMedia(mime)` — video/* + audio/*
...
| Blob URL 漏洩 | ダウンロード時 100ms 後に revoke | プレビュー用 blob URL は明示的に revoke されていない |
| 別窓からの parent 参照 | `noopener` 指定 | なし（十分） |
| SVG 内スクリプト | Data URI で表示（sandbox なし） | SVG の `<script>` が実行される可能性あり |
...
```
Phase 1 (SVG) ← セキュリティ修正、最優先
Phase 2 (Blob lifecycle) ← メモリ改善
Phase 3 (UX) ← 機能改善
