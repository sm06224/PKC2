...
> **`?pkc-debug=<feature>` を URL に付けてもう一度操作すると、画面に当該 feature の overlay が出る。困った瞬間に 🐞 ボタン(右上 ⚙ の右)を押すと、AppState / 直近 dispatch / errors / DOM / 環境情報が `pkc2-debug-<ISO-ts>.json` としてダウンロードされ、ユーザーはそれをそのまま issue / PR / Claude conversation にドラッグ添付できる。**
...
PR #211 finalize(2026-05-02、reform-2026-05 段階 β)時点の確定仕様:

- **第 1 候補(default)**: 合成された `<a download="pkc2-debug-{ISO-ts}.json">` クリックで JSON ファイルをダウンロード。ブラウザの download manager がファイル名込みで保存する。ファイル名は filesystem-safe な ISO 形式で sortable
- **失敗時**(engine OOM / sandboxed iframe / DOM unavailable など): toast「Failed to generate debug report — the container may be too large.」を出して通知。modal は使わない(PKC2 はメインウィンドウ越しの backdrop modal を禁止)

旧仕様(α): `navigator.clipboard.writeText` でクリップボードコピー → fallback modal。これは PR #211 で **download 方式に置換**された:
- clipboard API は HTTPS / secure context 要件が browser ごとに異なる
- pasted JSON は GitHub issue / Slack で formatting が崩れがち
- 新タブで blob URL を開いて Ctrl+S させる中間案も Firefox / Safari でファイル名が UUID になる + 一部 engine では保存自体不安定

`?pkc-debug-download=1` の opt-in flag は不要になった(default が download)。
...
| 段階 | 範囲 | 完了基準 |
|---|---|---|
| α | `runtime/debug-flags.ts` + 🐞 ボタン + 最小 dump(env / phase / selectedLid / container counts) | URL flag が読める、🐞 が動く |
| β | `recordDebugEvent` ring buffer + content opt-in + errors[] + replay seed + commit + storage + container fingerprint + 1 MiB total cap + JSON download | schema 3、recent[] 100 件 + errors[] 10 件 + replay(content opt-in)が download される |
