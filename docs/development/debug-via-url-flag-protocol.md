...
- `container.entries[].body` は**含めない**(本文は機密の可能性)。代わりに `entryCount` と archetype 別のサンプリングだけ。
- `container.assets` の base64 本体は**含めない**(`assetKeys` の名前一覧のみ)。
- `screenshot` は default OFF。`?pkc-debug-shot=1` で明示的に opt-in したときのみ。

これは「**ユーザーが書き換えなくても貼って安全な report**」を作るための原則。
