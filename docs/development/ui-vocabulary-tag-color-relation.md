...
**Status**: docs-only UI vocabulary source of truth。本ドラフトの label は Tag / Color tag 実装 slice が実際に着地するまで "予約語" 扱いだが、それ以降は UI・manual・dev docs で **同じ語を同じ意味で** 使うための正本となる。
