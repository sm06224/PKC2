...
| 2026-07-08〜14 | **user 発意の改善バッチ着地**(凍結の例外 = user 明示要望): ① structure DSL v2 + @alias(#911)/ ④ 差分保存 split 形式(opt-in、#912)/ ⑤ AI 整理プラン連携 pkc-ext 拡張(#913 設計 → #914)。マニュアル・about・changelog 整備で **v2.5.0**(#915/#916)。minimap は user 判断で既定 OFF |
| 2026-07-15〜18 | **不具合改修 + user 要望ラウンド**: text 末尾追記の画像貼付 fix(#917)/ HTML 貼付の Markdown 復元(#918 opt-in → 既定 ON 化、撤去台帳 #919)/ マルチメディア埋め込みプレーヤー + 録音・画面収録(#921/#922 → #923)/ OneNote 送信拡張 v0(#924 設計 → #925)/ launcher 刷新(拡張自動表示 + URL タイル #926-#927、タイル整理 #928-#929 → **DnD 主体へ user 指摘で刷新** #931)/ タブ 1 テンポ遅れ fix + 右クリック一覧 + compact labels(#932-#933)。差分保存の実測ベンチ + 既定 ON 判断材料 doc(#930、判断は user 待ち)。フル smoke 280 pass で回帰なし確認 |
