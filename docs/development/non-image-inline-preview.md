...
preview が展開された場合、元の chip link は **非表示** にする（削除はしない）。
preview 失敗時に chip を再表示するため。
...
| MIME が対象外 | chip のまま |
| PDF viewer 非対応ブラウザ | `<object>` 内の fallback テキスト表示 |
...
| リスク | 対策 |
|--------|------|
| blob: URL の CSP 制約 | single-HTML app は CSP なし (no meta tag)。blob: URL は使用可能 |
