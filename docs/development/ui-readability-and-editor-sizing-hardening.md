...
| C | 編集モード textarea のサイズ追従が不自然 | UX 不具合 | 高 | **COMPLETED** |
| A | Markdown rendered HTML の行間/密度 | UX 改善 | 中 | **COMPLETED** |
...
#### 実装記録（COMPLETED, 2026-04-12）

**センターペイン**:
- `detail-presenter.ts:87-91`: `bodyArea.rows = Math.max(15, lineCount + 3)`（`lineCount = entry.body.split('\n').length`）
- `resize: vertical` 維持（ユーザ手動調整可）
- split-editor の `min-height` は既存 200px のまま（下限としては 15 行 ≒ textarea 自身の height で十分）

**Entry window**:
- `.pkc-window-content` に `display: flex; flex-direction: column` を追加（edit-pane の flex:1 チェーン成立）
- `.pkc-editor[data-pkc-wide="true"]`: `max-width: none; display: flex; flex-direction: column; flex: 1; min-height: 0`
- `.pkc-editor-body[data-pkc-viewport-sized="true"]`: `flex: 1; min-height: calc(100vh - 12rem)`
- edit-pane の `data-pkc-wide` 属性は **非構造化 archetype のみ** に付与（useStructuredEditor=false）
- 非構造化 textarea は `rows="10"` 削除 + `data-pkc-viewport-sized="true"` のみ
- 構造化 archetype（textlog/todo/form）の hidden textarea は `rows="10"` のまま（display:none のため影響なし）

**テスト**:
- `tests/adapter/detail-presenter.test.ts` Slice C 4 件（min 15 / lineCount+3 / empty / threshold）
- `tests/adapter/entry-window.test.ts` Slice C 6 件（viewport-sized / wide 属性 / structured archetypes 不変 / inline CSS / window-content flex）
...
**CSS 変数定義** (`src/styles/base.css` `:root`):
- `--font-body: var(--font-sans)` — markdown body font alias。現状はモノスペース維持（PKC2 terminal テーマ一貫性）
- `--radius-sm: 1px` — inline code / fenced code / asset chip 用の小半径
- `--c-text: var(--c-fg)` — 標準テキスト色エイリアス
- `--c-text-dim: var(--c-muted)` — dim テキスト色エイリアス（blockquote 等）

いずれも `var()` chain による解決のため、`#pkc-root[data-pkc-theme="light|dark"]` の
`--c-fg` / `--c-muted` 再定義で自動追従する（light/dark mode 整合を壊さない）。

**密度調整**:
- `.pkc-md-rendered { line-height: 1.45 }` を明示（従来は `.pkc-view-body` 由来の 1.5 を継承）
- `.pkc-md-rendered pre { line-height: 1.35 }` を追加（code block はより高密度）
- `.pkc-view-body` の `line-height: 1.5` はそのまま（非 markdown pre 表示の互換維持）
- paragraph / list / heading の margin は不変（既存で十分タイト）
- font-size は不変（0.8rem）
- task checkbox の `vertical-align` / `margin-right` は不変

**scope 外（意図的に触らない）**:
- `entry-window.ts` の inline CSS
- `markdown-render.ts` の HTML 生成ロジック
- PDF / sandbox preview（Slice B 済）
- editor sizing（Slice C 済）
- font-size 設定 UI / typography theme switch

**テスト** (`tests/styles/markdown-readability.test.ts`): Slice A 11 件
- CSS 変数 4 件（`--font-body` / `--radius-sm` / `--c-text` / `--c-text-dim`）
- line-height 2 件（`.pkc-md-rendered` 1.45 / `.pkc-md-rendered pre` 1.35）
- regression guard 5 件（font-family / blockquote color / task checkbox / paragraph / list item）
