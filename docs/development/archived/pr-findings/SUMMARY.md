...
| #198 — | (live tree 残置:real keystroke smoke 不足) | textarea key helpers(Enter list / bracket pairs / skip-out / Tab indent) |

## 残置 4 件(deficit register、`../../INDEX.md` で追跡)
...
- `editor-key-helpers-pr198-findings.md`(real keystroke smoke 不在)
- `sublocation-prefix-cache-pr190-findings.md`(no-match cache UX-effect smoke 不在)

これらは Phase 1B(test 補強)後に archive 候補となる。
