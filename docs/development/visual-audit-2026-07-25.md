...
### B3 🟡判断 spreadsheet の長文セルで行高が爆発

- 長い文字列のセルが 1 文字ずつ縦積みになり、行高が数十倍になる
- 列幅の既定方針(`max-width` + ellipsis / 折り返し / 横スクロール)は
  **表計算としての体験設計**の判断。「見切れてもいいから 1 行」か
  「全部見えるが縦に伸びる」かは好みが分かれる

### B4 🟡判断 attachment: `text/plain` は「✎ 編集」できるのに「プレビュー不可」

- `attachment-presenter.ts:801-805` — `previewType === 'none'` で
  「Preview is not available for this file type」を表示
- しかし直前の PR(#1005)で **text 添付のその場編集**を入れたので、
  同じカードに「✎ 編集」ボタンが並ぶ。**編集はできるが閲覧はできない**という
  ちぐはぐな状態
- プレビュー対応 mime を広げる = **機能追加**なので凍結方針の対象。
  ただし「編集できるものは見られる」整合性の回復とも読める。user 裁定

### B5 🟡判断 Kanban カードの todo description を markdown render するか

- 現状 plain text。長文 description が生の記法のまま並ぶ
- render すると Kanban が重くなる / カード高さが不揃いになる。方針判断
...
| **PR-B3〜B5** | spreadsheet 列幅 / attachment プレビュー / Kanban markdown | **裁定待ち** | 各 S〜M |
