...
- **Folder context**: 現行 auto-folder-placement（`docs/development/auto-folder-placement-for-generated-entries.md`）の `TODOS` 自動配置を **そのまま踏襲**。context 引き継ぎで新しい判定ロジックは追加しない
