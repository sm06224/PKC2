...
- **pane 再トグル shortcut のキー衝突**: 既存 keydown 優先度
  (`docs/development/completed/text-textlog-editing-ux-consolidation.md`) と衝突しない
...
### Slice 4 — TEXTLOG → TEXT 変換 (**実装済み**)

- TEXTLOG viewer にログ行 multi-select UI (選択モード制御)
- 「Selected logs → new TEXT」action (TEXTLOG 内 toolbar)。Data menu は
  汚さない
- `features/textlog/textlog-to-text.ts` (純粋関数)
- preview modal → commit (`CREATE_ENTRY` + `COMMIT_EDIT` の既存 pipeline を再利用)
- backlink 形は補助 spec 1 で確定
- **依存**: Slice 2 (変換後の TEXT に TODO embed や TEXTLOG embed が含まれ
  たときの意味論が確定している必要あり)
- **実装**: `src/features/textlog/textlog-to-text.ts`,
  `src/adapter/ui/textlog-selection.ts`,
  `src/adapter/ui/textlog-preview-modal.ts`,
  `src/adapter/ui/textlog-presenter.ts` (選択 UI) ,
  `src/adapter/ui/action-binder.ts` (begin-textlog-selection /
  cancel-textlog-selection / open-textlog-to-text-preview /
  cancel-textlog-to-text / confirm-textlog-to-text)

### Slice 5 — TEXT → TEXTLOG 変換 (**実装済み — v1 範囲**)

- TEXT viewer に「📝 → TEXTLOG」action (action bar More メニュー配下)
- 分割単位: heading (ATX `#` / `##` / `###`) または手動 (`---`) のみ
  (preview modal 内の radio で択一、同時指定は不可)
- 投入先: **v1 は新規 TEXTLOG 作成のみ**。既存 TEXTLOG への append は
  将来拡張として保留
- `features/text/text-to-textlog.ts` (純粋関数)
- **実装**: `src/features/text/text-to-textlog.ts`,
  `src/adapter/ui/text-to-textlog-modal.ts`,
  `src/adapter/ui/renderer.ts` (action bar trigger),
  `src/adapter/ui/action-binder.ts`
  (open-text-to-textlog-preview / cancel-text-to-textlog /
  confirm-text-to-textlog + split-mode radio handler)
...
- `docs/development/completed/folder-structure-restore.md` — FOLDER 構造
- `docs/development/completed/text-textlog-editing-ux-consolidation.md` — keydown
