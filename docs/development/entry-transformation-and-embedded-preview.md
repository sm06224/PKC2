...
| TEXT              | ✅   | ✅       | ✅                                    |
| TEXTLOG           | ✅   | ✅       | ✅                                    |
| TODO              | ✅   | ❌       | **✅** (status + date + description)  |
| FOLDER            | ✅   | ❌       | ❌ (description markdown 化まで)      |
| attachment / form | ✅   | ❌       | ❌                                    |
...
- **TODO embed の描画粒度**: status のみ / date のみ / full の 3 段階を出すか。
  初期案は full のみ。
- **FOLDER description markdown 化の衝突**: 既存データに plain text として
  `#` や `---` が保存されている場合、render 後に意味が変わる。slice 3 投入前
  に影響エントリ数の見積もりが必要。
...
### Slice 3 — TODO / FOLDER description の markdown 化

- `todo-presenter` / `folder-presenter` の viewer path を `renderMarkdown()` に
  差し替え
- editor は textarea 維持
- TODO description 内の entry ref / asset ref を closure 走査対象に追加
- FOLDER description は markdown render のみ (embed 対象化は将来)
- 既存 plain text が heading / list 記法を含まないことの migration audit
