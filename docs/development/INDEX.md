...
Last updated: 2026-04-24（Phase 2 Slice 1 実装: Normalize PKC links pure scanner + preview model、features 層 pure function + 47 tests、UI / apply reducer は後続 slice）.
...
| 156 | `src/features/link/migration-scanner.ts` (NEW) + tests | Phase 2 Slice 1: Normalize PKC links pure scanner + preview model | 2026-04-24 | Phase 2 の実装フェーズ最初のピース。spec `docs/spec/link-migration-tool-v1.md` §5(scanner)/ §6(preview model)/ §10(tests)を features 層 pure function として実装。`buildLinkMigrationPreview(container, options?) → { candidates, summary }` と convenience `scanLinkMigrationCandidates(container, options?)` を export。Candidate 4 種(A 空 label / B legacy log fragment row-id match 時のみ / C same-container Portable Reference / D 画像 embed opt-in only)を分類、label 合成は `link-paste-handler` の `resolveLabel` と同じ規約を pure に再実装(entry title、log snippet 40 char truncate + `…` + timestamp fallback、attachment name、`(untitled)` 最終 fallback)、escape は inline helper。grammar は既存 `parseEntryRef` / `parsePortablePkcReference` / `parseTextlogBody` を再利用(二重実装なし)。非干渉: URL scheme allowlist(12 種 + 未知 scheme)/ cross-container `pkc://<other>/...` / non-empty canonical / unknown fragment / image-form on entry/portable / malformed body / 全てテストで固定。Code block 非干渉は `maskCodeRegions` で fenced(``` / ~~~)+ inline code を blank 置換、オフセット保持でマッチ位置は元テキスト基準。archetype scope: text / textlog(row 単位で `parseTextlogBody` → `row.text` 個別 scan)/ folder / todo / form、attachment + system-reserved(`__*__` / `system-*`)は skip。Deterministic order: container.entries → entry 内 offset 昇順 → textlog row 配列順 → row 内 offset 昇順(テストで pin)。Malformed body(JSON 破損 / 不均衡 brackets)は throw せず silent skip。**47 unit tests** 新規(A-D 各候補 + 12 scheme 非干渉 + code block / inline code / tilde fence / mask-through / archetype scope 5 種 + deterministic order 2 + summary + malformed 4 + facade)、合計 5466/5466 pass(+47)。DOM / state / I/O 非依存、bundle.js は未 wire-in のため tree-shake で不変(665.92 KB / 42.3% OK)。次 slice(UI dialog / apply reducer)は本 scanner を consumer として利用 |
