...
sidebar keyboard navigation は Phase 1–5 で tree navigation が完成形に近づいた。
...
Left/Right 対称性: Right で expand → Right で子へ入る → Left で collapse → Left で親に戻る
...
| # | 候補 | ユーザ価値 | コスト | リスク | 備考 |
|---|------|----------|--------|--------|------|
| 1 | non-folder の Arrow Left で parent 移動 | 中 — folder 以外からも親に戻れる | 小 (getStructuralParent 再利用、archetype guard 緩和のみ) | 低 — sidebar 限定 | folder 限定を解除する拡張 |
