...
| 🧠 AI | **✓ Phase 1 完了**(pgc-147 frontmatter suggestion + pgc-148 abandoned warning + pgc-149 broken link summary、flag `shell.inspector_ai_local_enabled` opt-in、Phase 2 = A 群 2/5/6/7/8 は後続 stack) |
...
- **pgc-148 着地** ✅:A 群 4 = `abandoned entry warning`(updated_at が 30
  ...
  link-index 再利用)
- **pgc-149 着地** ✅:A 群 3 = `broken link summary`(本文 `entry:<lid>`
  参照の target が container に存在しない broken を集約、Inspector に 🔗
  Broken links danger box で表示、target lid 列挙 + dismiss button)。
  `src/features/ai/broken-link-summary.ts` + `inspector-ai-tab.ts` 拡張。
  12 件 case matrix + adapter test +6 件、bundle +2 KB。**Phase 1 完了**
...
| 2026-05-24 | **pgc-149 着地** ── Phase 1 A3(broken link summary)を同 flag で実装。本文 `entry:<lid>` 参照の target 不在を集約 + 🔗 danger box + target chip 列挙 + dismiss。`src/features/ai/broken-link-summary.ts` 新規。**Phase 1 完了**(frontmatter suggestion + abandoned warning + broken link summary、3 機能 1 flag)。次は Phase 2(A 群 2/5/6/7/8、重複検出 / 循環 / outline lint / tag バランス / archetype mismatch)を後続 stack で順次 |
