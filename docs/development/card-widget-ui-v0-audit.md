...
- **Status**: 推奨 **Slice 5.0(minimal chrome)を即着手 → Slice 5.1(excerpt)→ Slice 5.2(thumbnail)→ Slice 6(variants)** の 4 段階。本 audit は **docs-only**(2026-04-25 着地)、**Slice 5.0 + 5.1 implementation も同日着地**(5.0: `src/features/card/widget-presenter.ts` + `src/adapter/ui/card-hydrator.ts` 新規追加、4 presenter から `expandTransclusions` の直後に呼び出し、CSS で `.pkc-card-widget` chrome、19 件の hydrator test で missing / cross-container / malformed / re-hydration / archetype 別 / Slice 4 contract 維持を pin。5.1: `src/features/card/excerpt-builder.ts` 新規追加、archetype 別の plain-text excerpt source、markdown flatten + `<script>`/`<style>` block content drop + nested-paren link/image 対応、35 件の builder test + 10 件の hydrator extension test で XSS 安全性 + textlog fragment routing + length cap + null-on-empty を pin、`widget-presenter` を additive 拡張(`entry?: Entry` + `parsed?: ParsedEntryRef` を ok status のみで返す)、CSS で `.pkc-card-widget-excerpt` 追加。Slice 5.2 / 6 は本 audit のとおり段階的に着地。
