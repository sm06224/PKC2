...
## 5. Attachment Preview / Sandbox (実装済み)

全 MIME 種別に対応したプレビューシステムを `classifyPreviewType()` で統一管理:

| 種別 | MIME | 方式 | 状態 |
|------|------|------|------|
| image | image/png, jpeg, gif, webp, svg, bmp, ico | inline `<img>` (base64 data URI) | **実装済み** |
| pdf | application/pdf | `<object>` + Blob URL + "Open in New Window" ボタン | **実装済み** |
| video | video/mp4, webm, ogg | `<video controls>` + Blob URL | **実装済み** |
| audio | audio/mpeg, wav, ogg, webm | `<audio controls>` + Blob URL | **実装済み** |
| html | text/html | `<iframe sandbox>` + Blob URL + "Open in New Window" ボタン | **実装済み** |
| none | その他 | download のみ | **実装済み** |

### HTML Sandbox ポリシー
- デフォルト: `sandbox="allow-same-origin"` (scripts 無効、forms 無効、popups 無効)
- "Open in New Window" ボタンで Blob URL を別窓表示（フル機能）
- 将来: per-file sandbox allow 設定を右ペイン UI で切替可能にする
...
| Phase 1 | markdown-it による完全 CommonMark レンダリング | **実装済み** |
| Phase 2 | PKC アセット参照 → BLOB URI 解決 | 未実装 |
| Phase 3 | markdown-it plugin (KaTeX, highlight.js 等) | 未実装 (基盤済み) |
| Phase 4 | 汎用 HTML レンダリング + sanitization | 未実装 |

markdown-it のプラグインシステムにより Phase 3 は `getMarkdownInstance().use(plugin)` で追加可能。
Phase 2 以降は単一 HTML 成約との整合が必要。

## 7. 別窓編集ウィンドウ (実装済み)

ダブルクリックで entry を別ブラウザウィンドウで開く。

| 機能 | 状態 |
|------|------|
| Markdown レンダリング表示 | **実装済み** |
| 編集モード (Source/Preview 切替) | **実装済み** |
| 保存 (postMessage → 親ウィンドウ) | **実装済み** |
| 競合検出 (updated_at 比較) | **実装済み** |
| 重複ウィンドウ防止 (同一 lid) | **実装済み** |

### 競合検出フロー
1. 別窓を開いた時点の `entry.updated_at` を `openedAt` として記録
2. 別窓から Save 時、親ウィンドウが現在の `entry.updated_at` と比較
3. 不一致なら conflict banner を別窓に表示
4. 保存は実行される（上書き）。ユーザーは右ペインの revision history から戻し可能
