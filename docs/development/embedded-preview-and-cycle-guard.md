...
| reason    | 文言                                           | 発生条件                     |
|-----------|------------------------------------------------|------------------------------|
| `cycle`   | `(cyclic embed blocked: entry:<lid>)`          | visited set で cycle 検知    |
| `self`    | `(self-embed blocked: entry:<lid>)`            | root と同じ lid を embed     |
| `missing` | `(missing entry: entry:<lid>)`                 | 対象 lid が container に無い |
| `archetype` | `(embed not supported for <archetype>: entry:<lid>)` | attachment / form / opaque / generic |
| `depth`   | `(embed depth limit: entry:<lid>)`             | depth > 1 の embed 試行      |
...
- `tests/adapter/transclusion-todo-embed.test.ts`
  ...
  - 非対応 archetype (attachment / form / opaque) embed →
    `(embed not supported for <archetype>)` placeholder
  - depth 超過: embed の中の embed → `(embed depth limit)` placeholder
...
- `tests/adapter/transclusion-depth-limit.test.ts` (既存に追加)
  - depth ≤ 1 不変を pin。depth 2 の embed 試行 → `(embed depth limit)`
