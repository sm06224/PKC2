...
**アプリの JS にホットスポットが無くなった。** 残るのはブラウザ内部の処理
(5.9MB バンドルのパース、15000 行のレイアウト)で、これは性質の違う問題である。

## 6. アーキテクチャ検討への含意

**この結果は、進行中のストレージ再設計の前提を変える。**

1. **boot が遅い理由はストレージではなかった。** 固い DB / 追記型 WAL /
   パーティション化のいずれも、実測で 1.5% しかない部分を改善する提案になる
