...
修正後、**同条件の boot は大幅に速くなった**。
ストレージのアーキテクチャには一切触れていない。

| | 修正前 | 修正後 |
|---|---|---|
| boot(15000 entries / 45000 revisions / 76MB) | **27,201 ms** | **936 ms** |

> 🔴 **訂正(2026-07-26)。初版はここに「27,201 ms → 3,110 ms(8.7 倍)」と書いていたが、
> これは別々のハーネスの数字を並べたものだった。**
>
> - 27,201 ms … `tests/bench/storage-write-io.mjs`
>   (同値が [`storage-write-io-bench-2026-07-25.md`](./storage-write-io-bench-2026-07-25.md) §2-b にある)
> - 3,110 ms … scratchpad の CDP プロファイル用ハーネス(**別実装**)
>
> **倍率を出すなら同じ計器で測った 2 点でなければならない。**
> 同一ハーネス(`storage-write-io.mjs`)で測り直した修正後の値は **936 ms**。
> 上表はそちらに差し替えた。以下 §5 に出てくる 3,110 / 5,291 ms は
> **プロファイル用ハーネスの値**であり、上表とは混ぜて読めない。
>
> `perf-measurement` skill に「**倍率は同一計器の 2 点でのみ出す**」として収録済み。
...
1. **boot が遅かった理由はストレージではなかった。** 修正前の 22.9 秒のうち
   IDB は 1.5% しかなく、O(N×M) の索引化だけで大幅に縮んだ
   (同一ハーネスで測り直した対は **27,201 → 936 ms**。⚠ 初版がここに書いていた
   「8.7 倍」は**別ハーネス同士の比較**で、§冒頭の訂正ブロックで撤回済み)。
