...
- save 後: child 側 `renderBodyView(originalBody)` → per-log-entry 分解 + `data-pkc-log-id` wrap

全 3 経路で per-log-entry 構造が維持される。badge DOM 導出も全経路で正しく動作する。
（#62 で `renderBodyView()` を導入し、save 経路を修正済み。）
