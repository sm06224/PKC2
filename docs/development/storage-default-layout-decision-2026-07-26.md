...
## 5-b. 🔴 user 裁定 ── `lazy_entry_bodies` の退役(2026-07-26)

> user 裁定(2026-07-26):
> 「**lazy_entry_bodies は効果が少なく、リスクが多いなら廃止したい /
> 今回のこの騒動を見てくれ、とても気分が悪いよ僕は /
> わかった 3 ヶ月後に廃止する方向性で調整に入りましょう /
> まずは導線の封鎖と戻し道をつけてください /
> 3 ヶ月の間にユーザーが一度でも上書きすれば、安全な道に戻る**」

### 実施したこと

1. **導線の封鎖** ── `defineFlag` に `retired` を新設(`core/flags.ts`)。
   - **どの source から指定されても既定値に落ちる**(URL / container の
     `__flags__` / Inspector 編集をすべて無視)
   - `getRegisteredFlags()` が返さない = Inspector の一覧から消える
   - 値の解決と一覧の**両方**を塞ぐのが要点。UI から消すだけでは URL flag で
     有効化できてしまう(2026-07-25 に移行前 ZIP ゲートで実際に踏んだ穴)
2. **戻し道を安全にした** ── `save()` に本文の未読ガードを追加。
   これは §6 の S3 そのものであり、**退役の一括変換が通る経路**だった。
   未読の本文は storage から復元してから書き、**1 つでも復元できないなら
   inline 変換を中止して現状のまま残す**(layout 5 のままでいる方が、
   空で焼き付くより常に安全)
3. **定義は消していない。** getter が生きていないと、既に有効化された環境が
   「既定値へ戻る = 安全な形式へ書き戻る」経路ごと失われる

### 3 ヶ月後の廃止範囲は未確定

`storage-v3-redesign-2026-07.md` §A.7 の **user 出典タグ付き指示**
(「ゆるいストリーミング圧縮とチャンクパックはスケールのために必須」/
実デバイス書込 1/4.9 の実測)は **撤回されていない**。
よって **segments 実装そのものは残す**。今回塞いだのは user 導線だけであり、
**実装を消すかどうかは別途 user 裁定を要する**。

regression test: `tests/adapter/lazy-bodies-retirement.test.ts`(5 件)
── 導線封鎖 2 件は退役前に落ち、「空で焼かない」1 件はガード追加前に落ちる。
残る 2 件は既存の双方向設計が守っていた部分の回帰確認。
