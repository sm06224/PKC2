...
- 🔴 **VmRSS 合計から倍率・削減量を書かない**(共有ページの二重計上。chromium は
  ...
  ⚠ **この規律を書いた本人が破った**(2026-07-28):別 doc に「計器固定費 ~0.5GB」と
  RSS 合計由来の数字を書き、user の PWA 実測(**200MB に届かない**)と食い違った。
  user 指摘「あなたの 500MB という数字はどこから持って来ましたか?**削り込める部分を
  過小に見積もっています**」── 正しい。PSS/USS で測り直すと **基盤 USS 187.9MB /
  PKC2 自身の取り分 +206.1MB**(3,000 entries)で、**app 側のほうが大きかった**。
  「床が支配的だからもう削れない」は**分母を間違えた結論**だった。
  基盤コスト・削減量を語るときは **`tests/bench/pwa-base-cost.mjs`(PSS/USS・
  `--app` の PWA ウィンドウ・headed)** を使う。Windows のタスクマネージャの
  「プライベート ワーキング セット」は **USS 相当** ── user と同じ指標で話すこと
