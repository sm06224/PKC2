 ...
 *
 * ## 🔴 推定値に**中央値を使ってはいけない**(2026-07-29、実機報告から実測)
 *
 * 当初は「平均だと表 1 個のような外れ値に引きずられる」という理由で中央値を
 * 採っていた。**これが user 実機の「一生スクロールできない」の原因だった。**
 *
 * ブロック高の分布は**二峰性**である ── 見出し / 段落 / 箇条は ~21px、
 * 表 / コードは ~40px。中央値はその**谷の上に乗る**ので、実測が 1 個増えた
 * だけで 21 ↔ 29.4 と山を飛び移る。推定は未測定の全ブロック(~100 個)に
 * 一斉に効くので、**総高が 8,296 ↔ 10,637 を往復する**。scroll 範囲が
 * 縮むたびにブラウザが `scrollTop` をクランプするため、ホイールを回し続けると
 * 「進む → 範囲が縮む → 戻される」が終わらない。実測(ホイール 20 回):
 *
 * | 実測数 | 中央値 | 平均 | 総高 |
 * |---|---|---|---|
 * | 86/360  | **21**   | 29.6 | 8,296 |
 * | 112/360 | **29.4** | 29.9 | 10,637 |
 * | 140/360 | **21**   | 29.7 | 8,772 |
 * | 166/360 | **29.4** | 29.8 | 10,659 |
 *
 * **平均は 29.6〜30.0 の 1.3% 幅に収まっている。** そもそも推定したいのは
 * 「よくあるブロックの高さ」ではなく**総和**であり、総和の不偏推定量は
 * 平均 × 個数であって中央値 × 個数ではない。外れ値耐性は
 * *位置決め* に要る性質で、*総和* には要らない ── 目的を取り違えていた。
 *
 * ## 🔴 測るのは「高さ」ではなく**送り幅(pitch)**である(同上)
 *
 * `last.bottom - first.top` はブロックの**インクの高さ**しか測らない。
 * 文書の高さには**ブロック間のマージン**が入るので、インクの総和は
 * 常に足りない ── 実測で平均 29.8 × 360 = 10,728 に対し真値 13,032、
 * **2,300px(18%)不足**していた。マージン ~6.4px × 360 個にぴたり一致する。
 *
 * 総和が合う量は「次のブロックの上端 − このブロックの上端」= **送り幅**。
 * `measureVisibleBlockHeights` はこちらを返す(`center-block-dom.ts` 参照)。
 */
...
/**
 * 平均(空なら null)。**中央値は使わない** ── 上の doc の通り、推定したいのは
 * 総和であり、二峰分布の中央値は山を飛び移って総高を往復させる。
 */
...
export function withMeasured(
  ...
  return {
    count: metrics.count,
    heights,
    estimate: mean(known) ?? CENTER_BLOCK_DEFAULT_ESTIMATE,
