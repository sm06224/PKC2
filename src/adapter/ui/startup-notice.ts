...
export const STARTUP_NOTICES: readonly StartupNotice[] = [
  {
    id: '2026-07-29-center-window',
    date: '2026-07-29',
    title: '長い文書を開くのが速くなりました(中央ペインを見えている範囲だけ描くように)',
    items: [
      '長い本文を開いたときの待ち時間を短くしました。画面に入っている範囲だけを描き、スクロールに合わせて描き足します(120KB の文書で、開くまでのメインスレッド占有が約 1.4 秒 → 約 0.3 秒)',
      '一度開いた文書は描画結果を覚えておき、開き直したときに作り直しません(同じ文書を行き来したときのメインスレッド占有が約 0.24 秒 → 約 0.12 秒)',
      '見出しの折りたたみが、スクロールしても開き直らなくなりました',
      '⚙ 設定の Flags から `center.block_window` / `center.render_cache` を OFF にすると、従来の描き方に戻せます',
    ],
  },
  {
    id: '2026-07-28-todo-unread-guard',
    date: '2026-07-28',
    title: '安全策:読み込み中の TODO を操作しても、期日や説明が消えないようにしました',
    items: [
      '🔴 安全策: 本文をまだ読み込めていない TODO のチェックを切り替えたり、カンバン・カレンダーへドラッグしたりしたときに、期日・アーカイブ・説明文が失われる可能性がありました。今後は本文を読み込んでから反映します(読み込めなかった場合は変更せず、その旨をお知らせします)',
      'この問題が起きるのは、保存先を wasm-sqlite に切り替えている場合だけです。通常の保存先(IndexedDB)では発生しません',
    ],
  },
  {
