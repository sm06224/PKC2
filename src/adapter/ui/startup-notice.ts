...
export const STARTUP_NOTICES: readonly StartupNotice[] = [
  ...
  },
  {
    ...
    items: [
      ...
      '不具合修正: 高速起動形式(lazy_entry_bodies)をオンにしたあと従来形式へ戻したとき、分割・圧縮された本文と履歴の実体がブラウザ内に残り続け、保存領域を食っていた問題を修正しました(次回の保存で自動的に回収されます。データの読み書きには元から影響ありません)',
      '訂正: 高速起動形式(Flags Inspector の lazy_entry_bodies)について、マニュアルと flag 説明の記述を訂正しました。この形式は差分保存(differential_save)の上に載っているため、lazy_entry_bodies だけをオンにしても保存の形は変わりません(従来は「オンにすれば起動がほぼ一瞬」と読める書き方でした)。また 1000〜5000 エントリ規模の実測では起動の改善を確認できていないため、おすすめ度の表現も実測に合わせました。動作そのものは変わっていません',
    ],
  },
  {
