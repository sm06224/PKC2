...
export const STARTUP_NOTICES: readonly StartupNotice[] = [
  ...
  },
  {
    ...
    items: [
      '🔴 重要な不具合修正: 添付ファイルの多いデータ(数百 MB 規模)で、**起動直後にメモリが数 GB に達して落ちる**ことがある問題を修正しました。初回起動時の内部処理が、必要なのはデータの ID 1 個だけなのに**全添付をまとめてメモリに読み込んでいた**のが原因です(実測・添付 512KB × 100 件のデータで、起動時のメモリピークが約 300MB → 約 90MB)',
      '改善: 添付の一覧情報(サイズなど)を組み立てる起動後の処理が、**途中で中断しても続きから再開する**ようになりました。これまでは処理の完了時に 1 回だけ保存していたため、大きなデータで処理が完走できないと、**起動のたびに全添付を頭から読み直し**ていました。今後は少しずつ保存しながら進むので、数回の起動で必ず完了し、以後は読み直しません(あわせて処理に息継ぎを入れ、起動直後の負荷が集中しないようにしました)',
  ...
  },
  {
    ...
    items: [
      ...
      '終了の理由は速度ではなく **データの安全性**です。差分保存で保存されたデータを**古い pkc2.html で開くと、エントリ同士の「関連」が 0 件に見え、その状態で保存すると関連が実際に消えます**(編集履歴の並び順も失われます)。PKC2 は単一 HTML なので、古いファイルを手元に残して開く使い方が実際にありえます。この危険を残したままにはできないと判断しました',
  ...
  },
  {
    ...
    items: [
      ...
      '安全策: 内容を読み取れないスプレッドシートを開いたとき、警告を表示して**元データを保持する**ようになりました。これまでは空のシートと見分けがつかず、そのまま保存すると元データが空で上書きされていました(作り直したいときは「空のシートで作り直す」を押してください)',
  ...
  },
  {
    ...
    items: [
      ...
      '改善: **編集を保存したときの引っかかりも減らしました**。左ペインの各行は一度作ったものを使い回す仕組みでしたが、データを 1 文字でも保存すると **全行を捨てて作り直して**いました(履歴の件数や被参照数の表示が古くなる可能性があるため)。これらの表示は行ごとに安く比べられるので、**中身が実際に変わった行だけ**作り直すようにしました。実測(5000 エントリ)で、保存 1 回あたり画面が固まる時間が 749ms → 531ms。エントリ数が多いほど効果があります',
      ...
      '🔴 重要な不具合修正: 省容量形式(Flags Inspector の lazy_entry_bodies)をオンにしている場合、バックアップ ZIP や HTML 書き出しに **添付ファイルが 1 件も含まれない**ことがある問題を修正しました。起動直後など、本文がまだ読み込まれていない状態で書き出すと発生します(本文だけは入るため、開くまで気づけませんでした)。ストレージ形式を切り替える前に自動生成される「移行前バックアップ ZIP」も同じ経路だったため、**安全網そのものが添付なしになっていました**。この形式をお使いの場合、過去に取ったバックアップに添付が入っているかご確認ください(データ本体は無事です。取り直しをおすすめします)',
