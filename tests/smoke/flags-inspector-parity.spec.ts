...
test('flags inspector — URL `?pkc-flag=*` auto-opens at boot', async ({ page }) => {
  ...
  // flag は wave ごとに増える(過去:42 → 46 → 49 → 50、γ-A2/A3 で
  // shell.* / meta_pane.* 等が追加され現在 55+)。exact count の hardcode
  // は flag 追加のたびに壊れる brittle な設計だったため、**下限 assert**
  // に変更(50 未満 = flag が大量に消えた regression)。具体的な drift は
  // 下の key spot-check が検出する。
...
test('every Tier 0 flag row is reachable inside the inspector body', async ({
  ...
  // flag は wave ごとに増えるため exact count ではなく下限を assert
  // (本 test の主眼は「全 flag row が body 内で reachable」であり、
  // 行数そのものではない)。
  expect(snap.rowCount).toBeGreaterThanOrEqual(50);
