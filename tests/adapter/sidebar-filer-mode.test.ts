...
describe('filer モード sidebar (Phase γ-A1)', () => {
  ...
  it('flag 未設定(default tree、2026-06-02 user direction で復帰):filer モード sidebar は出ない', () => {
    // user direction 2026-06-02「左ペインのファイラ入れ替えも不要、ツリーの方が好評」
    // で sidebar.mode の default を 'filer' → 'tree' に flip。filer は opt-in。
    boot();
    expect(filerSidebar()).toBeNull();
  });

  it('flag tree:filer モード sidebar は出ない', () => {
