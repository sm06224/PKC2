...
const RELEASE_SUMMARY = {
  ...
  '2.1.1': {
    ...
    knownLimitations: [
      ...
      'Card widget excerpt + thumbnail + advanced variants (`compact` / `wide` / `timeline`) are still future work — Slice 1-4 (2026-04-25) shipped the parser, renderer placeholder, and click + keyboard wiring; Slice 5.0 (2026-04-25) hydrates the placeholder into a minimal chrome (archetype badge + entry title + missing / cross-container / malformed states) via `adapter/ui/card-hydrator.ts`. Excerpt (Slice 5.1), thumbnail (Slice 5.2), and per-variant layout (Slice 6) are deferred. Embed presentation beyond `![](entry:...)` transclusion is unchanged',
