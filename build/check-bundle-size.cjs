...
const BUDGETS = [
  ...
  // 2026-07-26(user 指示「**前から言ってるけど、予算はあくまで何かの手違いを
  //   検出するために設定してる。引き上げで問題ない**」):
  //   🔑 **この cap の目的は「サイズを守らせること」ではなく「手違いの検知」**である。
  //   重い dep を誤って取り込んだ / 生成物を取り違えた、のような **事故の桁**
  //   (数百 KB〜MB)を止めるための tripwire であって、通常増加を押さえ込む
  //   規律ではない。⇒ **残量が減ったこと自体は問題ではなく、引き上げてよい。**
  //
  //   このセッション(#1020–#1036)で 5840.0 → 5852.95 KB(+13 KB)となり
  //   5.75 MB 枠の 99.4%(残り 35 KB)に達した。通常増加のペースで cap に
  //   触れる = tripwire が本来の役目(事故検知)より先に鳴る状態なので、
  //   5.75 → **6.0 MB** に bump。現行 5852.95 KB に対し headroom ~291 KB で、
